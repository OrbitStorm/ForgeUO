...
namespace Server.Mobiles
{
    public partial class OmniAI : BaseAI
    {
        ...
        public override bool Think()
        {
            ...
            if (DateTime.Now > this.m_NextFieldCheck)
            {
                this.CheckForFieldSpells();
                this.m_NextFieldCheck = DateTime.Now + TimeSpan.FromSeconds(3);
            }

            if (DateTime.Now > this.m_NextCheckArmed)
            {
                this.CheckArmed(this.m_SwapWeapons);
                this.m_NextCheckArmed = DateTime.Now + TimeSpan.FromSeconds(12);
            }
        ...
        public override bool DoActionCombat()
        {
            ...
            if (DateTime.Now > this.m_NextPetCommand)
            {
                ...
                this.m_NextPetCommand = DateTime.Now + TimeSpan.FromSeconds(10);
            }
            ...
            if (this.TryToHeal())
                return true;
            else if (this.m_Mobile.Spell == null && DateTime.Now > this.m_NextCastTime && this.m_Mobile.InRange(combatant, 12))
            {
                this.m_NextCastTime = DateTime.Now + TimeSpan.FromSeconds(4);
