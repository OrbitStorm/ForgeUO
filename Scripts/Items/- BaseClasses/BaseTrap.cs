...
namespace Server.Items
{
    public abstract class BaseTrap : Item
    {
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            ...
            if (this.CheckRange(m.Location, oldLocation, 0) && DateTime.Now >= this.m_NextActiveTrigger)
            {
                this.m_NextActiveTrigger = this.m_NextPassiveTrigger = DateTime.Now + this.ResetDelay;

                this.OnTrigger(m);
            }
            else if (this.PassivelyTriggered && this.CheckRange(m.Location, oldLocation, this.PassiveTriggerRange) && DateTime.Now >= this.m_NextPassiveTrigger)
            {
                this.m_NextPassiveTrigger = DateTime.Now + this.PassiveTriggerDelay;
