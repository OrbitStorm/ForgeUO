...
namespace Server.Items
{
    public abstract class BaseExplosionPotion : BasePotion
    {
        ...
        private void Detonate_OnTick(object state)
        {
            ...
            object parent = this.FindParent(from);
            ...
            if (timer == 0)
            {
                ...
                this.Explode(from, true, loc, map);
                this.m_Timer = null;
            }
            else
            {
        ...
        private class ThrowTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                Effects.SendMovingEffect(from, to, this.m_Potion.ItemID, 7, 0, false, false, this.m_Potion.Hue, 0);

                if (this.m_Potion.Amount > 1)
                {
                    Mobile.LiftItemDupe(this.m_Potion, 1);
                }

                this.m_Potion.Internalize();
                Timer.DelayCall(TimeSpan.FromSeconds(1.0), new TimerStateCallback(this.m_Potion.Reposition_OnTick), new object[] { from, p, map });
            }
        }
    }
}