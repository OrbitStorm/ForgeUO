    ...
    public class Spellbook : Item, ICraftable, ISlayer
    {
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            if (dropped is SpellScroll && dropped.Amount == 1)
            {
                ...
                else if (this.HasSpell(scroll.SpellID))
                {
                ...
                {
                    int val = scroll.SpellID - this.BookOffset;

                    if (val >= 0 && val < this.BookCount)
                    {
                        this.m_Content |= (ulong)1 << val;
                        ++this.m_Count;
        ...
        public override void OnAfterDuped(Item newItem)
        {
            ...
            book.m_AosAttributes = new AosAttributes(newItem, this.m_AosAttributes);
            book.m_AosSkillBonuses = new AosSkillBonuses(newItem, this.m_AosSkillBonuses);
        }
        ...
        {
            spellID -= this.BookOffset;

            return (spellID >= 0 && spellID < this.BookCount && (this.m_Content & ((ulong)1 << spellID)) != 0);
        }
        ...
        public void DisplayTo(Mobile to)
        {
            ...
            if (Core.AOS)
            {
                if (ns.NewSpellbook)
                {
                    to.Send(new NewSpellbookContent(this, this.ItemID, this.BookOffset + 1, this.m_Content));
                }
                else
                {
                    to.Send(new SpellbookContent(this.m_Count, this.BookOffset + 1, this.m_Content, this));
                }
            }
            else
            {
                if (ns.ContainerGridLines)
                {
                    to.Send(new SpellbookContent6017(this.m_Count, this.BookOffset + 1, this.m_Content, this));
                }
                else
                {
                    to.Send(new SpellbookContent(this.m_Count, this.BookOffset + 1, this.m_Content, this));
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            if (this.m_EngravedText != null)
                list.Add(1072305, this.m_EngravedText); // Engraved: ~1_INSCRIPTION~
            ...
            list.Add(1042886, this.m_Count.ToString()); // ~1_NUMBERS_OF_SPELLS~ Spells
        }
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            this.LabelTo(from, 1042886, this.m_Count.ToString());
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (this.Parent == from || (pack != null && this.Parent == pack))
                this.DisplayTo(from);
            else
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch ( version )
            {
                ...
                case 0:
                    {
                        this.m_Content = reader.ReadULong();
