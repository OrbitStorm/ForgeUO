...
namespace Server.Items
{
    public class Bandage : Item, IDyable
    {
        ...
        {
            if (from.InRange(this.GetWorldLocation(), Range))
            {
    ...
    public class BandageContext
    {
        ...
        public void Slip()
        {
            this.m_Healer.SendLocalizedMessage(500961); // Your fingers slip!
            ++this.m_Slips;
        }
        ...
        public BandageContext(Mobile healer, Mobile patient, TimeSpan delay, bool enhanced)
        {
            ...
            this.m_Enhanced = enhanced;
        }

        public void StopHeal()
        {
            m_Table.Remove(this.m_Healer);
        ...
        public void EndHeal()
        {
            ...
            SkillName primarySkill = GetPrimarySkill(this.m_Patient);
            SkillName secondarySkill = GetSecondarySkill(this.m_Patient);

            BaseCreature petPatient = this.m_Patient as BaseCreature;

            if (!this.m_Healer.Alive)
            {
                healerNumber = 500962; // You were unable to finish your work before you died.
                patientNumber = -1;
                playSound = false;
            }
            else if (!this.m_Healer.InRange(this.m_Patient, Bandage.Range))
            {
                healerNumber = 500963; // You did not stay close enough to heal your target.
                patientNumber = -1;
                playSound = false;
            }
            else if (!this.m_Patient.Alive || (petPatient != null && petPatient.IsDeadPet))
            {
                double healing = this.m_Healer.Skills[primarySkill].Value;
                double anatomy = this.m_Healer.Skills[secondarySkill].Value;
                double chance = ((healing - 68.0) / 50.0) - (this.m_Slips * 0.02);

                if (((checkSkills = (healing >= 80.0 && anatomy >= 80.0)) && chance > Utility.RandomDouble()) ||
                    (Core.SE && petPatient is Factions.FactionWarHorse && petPatient.ControlMaster == this.m_Healer))	//TODO: Dbl check doesn't check for faction of the horse here?
                {
                    if (this.m_Patient.Map == null || !this.m_Patient.Map.CanFit(this.m_Patient.Location, 16, false, false))
                    {
                        healerNumber = 501042; // Target can not be resurrected at that location.
                        patientNumber = 502391; // Thou can not be resurrected there!
                    }
                    else if (this.m_Patient.Region != null && this.m_Patient.Region.IsPartOf("Khaldun"))
                    {
                    ...
                    else
                    {
                        ...
                        this.m_Patient.PlaySound(0x214);
                        this.m_Patient.FixedEffect(0x376A, 10, 16);

                        if (petPatient != null && petPatient.IsDeadPet)
                        {
                            Mobile master = petPatient.ControlMaster;

                            if (master != null && this.m_Healer == master)
                            {
                            ...
                            else if (master != null && master.InRange(petPatient, 3))
                            {
                                ...
                                master.SendGump(new PetResurrectGump(this.m_Healer, petPatient));
                            }
                            else
                            {
                                bool found = false;
                                ...
                                for (int i = 0; friends != null && i < friends.Count; ++i)
                                {
                                    ...
                                    if (friend.InRange(petPatient, 3))
                                    {
                                        ...
                                        friend.SendGump(new PetResurrectGump(this.m_Healer, petPatient));
                        ...
                        {
                            this.m_Patient.CloseGump(typeof(ResurrectGump));
                            this.m_Patient.SendGump(new ResurrectGump(this.m_Patient, this.m_Healer));
                        }
                    }
                }
                else
            ...
            else if (this.m_Patient.Poisoned)
            {
                this.m_Healer.SendLocalizedMessage(500969); // You finish applying the bandages.

                double healing = this.m_Healer.Skills[primarySkill].Value;
                double anatomy = this.m_Healer.Skills[secondarySkill].Value;
                double chance = ((healing - 30.0) / 50.0) - (this.m_Patient.Poison.Level * 0.1) - (this.m_Slips * 0.02);

                if ((checkSkills = (healing >= 60.0 && anatomy >= 60.0)) && chance > Utility.RandomDouble())
                {
                    if (this.m_Patient.CurePoison(this.m_Healer))
                    {
                        healerNumber = (this.m_Healer == this.m_Patient) ? -1 : 1010058; // You have cured the target of all poisons.
            ...
            else if (BleedAttack.IsBleeding(this.m_Patient))
            {
                ...
                BleedAttack.EndBleed(this.m_Patient, false);
            }
            else if (MortalStrike.IsWounded(this.m_Patient))
            {
                healerNumber = (this.m_Healer == this.m_Patient ? 1005000 : 1010398);
                patientNumber = -1;
                playSound = false;
            }
            else if (this.m_Patient.Hits == this.m_Patient.HitsMax)
            {
            ...
            else
            {
                ...
                double healing = this.m_Healer.Skills[primarySkill].Value;
                double anatomy = this.m_Healer.Skills[secondarySkill].Value;
                double chance = ((healing + 10.0) / 100.0) - (this.m_Slips * 0.02);
                ...
                if (chance > Utility.RandomDouble())
                {
                    ...
                    if (this.m_Patient.Body.IsMonster || this.m_Patient.Body.IsAnimal)
                        toHeal += this.m_Patient.HitsMax / 100;

                    if (Core.AOS)
                        toHeal -= toHeal * this.m_Slips * 0.35; // TODO: Verify algorithm
                    else
                        toHeal -= this.m_Slips * 4;
                    ...
                    this.m_Patient.Heal((int)toHeal, this.m_Healer, false);
                }
                else
                {
                    healerNumber = 500968; // You apply the bandages, but they barely help.
            ...
            if (healerNumber != -1)
                this.m_Healer.SendLocalizedMessage(healerNumber);

            if (patientNumber != -1)
                this.m_Patient.SendLocalizedMessage(patientNumber);

            if (playSound)
                this.m_Patient.PlaySound(0x57);

            if (checkSkills)
            {
                this.m_Healer.CheckSkill(secondarySkill, 0.0, 120.0);
                this.m_Healer.CheckSkill(primarySkill, 0.0, 120.0);
            }
        }
