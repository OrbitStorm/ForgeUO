...
namespace Server.Mobiles
{
	public class XmlSpawner : Item, ISpawner
	{
		...
		public bool DebugThis
		{
			get
			{
				return this.m_DebugThis;
			}
			set
			{
				this.m_DebugThis = value;
			}
		}
		...
		public int MovingPlayerCount
		{
			get
			{
				return this.m_MovingPlayerCount;
			}
			set
			{
				this.m_MovingPlayerCount = value;
			}
		}
		...
		public int FastestPlayerSpeed
		{
			get
			{
				return this.m_FastestPlayerSpeed;
			}
			set
			{
				this.m_FastestPlayerSpeed = value;
			}
		}
		...
		public int NearbyPlayerCount
		{
			...
			{
				int count = 0;
				if (this.ProximityRange >= 0)
				{
					foreach (Mobile m in this.GetMobilesInRange(this.ProximityRange))
					{
						if (m != null && m.Player)
							count++;
					}
		...
		public Point3D MostRecentSpawnPosition
		{
			get
			{
				return this.mostRecentSpawnPosition;
			}
			set
			{
				this.mostRecentSpawnPosition = value;
			}
		}
		...
		public XmlSpawnerGump SpawnerGump
		{
			get
			{
				return this.m_SpawnerGump;
			}
			set
			{
				this.m_SpawnerGump = value;
			}
		}
		...
		public bool SingleSector
		{
			get
			{
				return this.UseSectorActivate;
			}
		}
		...
		*/
		...
		public bool HasDamagedOrDistantSpawns
		{
			get
			{
				Sector ssec = this.Map.GetSector(this.Location);
					...
					for (int x = 0; x < so.SpawnedObjects.Count; x++)
					{
						...
						if (o is BaseCreature)
						{
							...
							if (b.Map != null && b.Map != Map.Internal)
							{
								...
								if (this.UseSectorActivate)
								{
									// is it in activatable range of the sector the spawner is in
									if (!this.InActivationRange(bsec, ssec))
									{
										return true;
									}
								}
		...
		public bool HasActiveSectors
		{
			get
			{
				if (!this.SmartSpawning || this.Map == null || this.Map == Map.Internal)
					return false;

				// is this a region spawner?
				if (this.m_Region != null)
				{
					List<Mobile> players = this.m_Region.GetPlayers();
				...
				if (this.sectorList == null)
				{
					...
					if (this.Parent != null)
					{
						if (this.RootParent is Mobile)
							loc = ((Mobile)(this.RootParent)).Location;
						else if (this.RootParent is Item)
							loc = ((Item)(this.RootParent)).Location;
					}
					...
					int x1 = this.m_X - bufferzone;
					int width = this.m_Width + 2 * bufferzone;
					int y1 = this.m_Y - bufferzone;
					int height = this.m_Height + 2 * bufferzone;
					...
					for (int x = x1; x <= x1 + width; x += Server.Map.SectorSize)
					{
						...
						{
							Sector s = this.Map.GetSector(new Point3D(x, y, loc.Z));
							...
							foreach (Sector olds in this.sectorList)
							{
								if (olds == s)
								{
									duplicate = true;
							...
							if (!duplicate)
							{
								this.sectorList.Add(s);

								if (GlobalSectorTable[this.Map.MapID] == null)
								{
									GlobalSectorTable[this.Map.MapID] = new Hashtable();
								}

								// add this sector and the spawner associated with it to the global sector table
								if (GlobalSectorTable[this.Map.MapID].Contains(s))
								{
									ArrayList spawnerlist = (ArrayList)GlobalSectorTable[this.Map.MapID][s];
									if (spawnerlist == null)
									{
										//GlobalSectorTable[Map.MapID].Remove(s);
										spawnerlist = new ArrayList();
										//GlobalSectorTable[Map.MapID].Add(s, spawnerlist);
										GlobalSectorTable[this.Map.MapID][s] = spawnerlist;
									}
								...
								else
								{
									...
									GlobalSectorTable[this.Map.MapID].Add(s, spawnerlist);
								}

								totalSectorsMonitored++;

								// add some sanity checking here
								if (this.sectorList.Count > MaxSmartSectorListSize)
								{
									this.SmartSpawning = false;

									// log it
									try
									{
										Console.WriteLine("SmartSpawning disabled at {0} {1} : Range too large.", loc, this.Map);

										using (StreamWriter op = new StreamWriter("badspawn.log", true))
										{
											op.WriteLine("{0} SmartSpawning disabled at {1} {2} : Range too large.", DateTime.Now, loc, this.Map);
											op.WriteLine();
										}
									}
									catch
		...
		public bool IsInactivated
		{
			get
			{
				return this.m_IsInactivated;
			}
			set
			{
				this.m_IsInactivated = value;
			}
		}
		...
		public bool PlayerCreated
		{
			get
			{
				return this.m_PlayerCreated;
			}
			set
			{
				this.m_PlayerCreated = value;
			}
		}
		...
		public bool OnHold
		{
			get
			{
				...
				if (this.m_KeywordTagList == null || this.m_KeywordTagList.Count == 0)
					return false;

				foreach (BaseXmlSpawner.KeywordTag sot in this.m_KeywordTagList)
				{
		...
		public string AddSpawn
		{
			...
			set
			{
				if (value.Length > 0)
				{
					...
					if (type != null)
						this.m_SpawnObjects.Add(new XmlSpawner.SpawnObject(str, 1));
					else
					{
		...
		public Skill TriggerSkill
		{
			get
			{
				if (this.TriggerMob != null && (int)this.m_skill_that_triggered >= 0)
				{
					return this.TriggerMob.Skills[this.m_skill_that_triggered];
				}
				else
				{
					return null;
			...
			set
			{
				if (value != null)
				{
					this.m_skill_that_triggered = value.SkillName;
				}
				else
				{
					this.m_skill_that_triggered = XmlSpawnerSkillCheck.RegisteredSkill.Invalid;
				}
			}
		}
		...
		public int SafeCurrentCount
		{
			get
			{
				return this.SafeTotalSpawnedObjects;
			}
		}
		...
		public bool FreeRun
		{
			get
			{
				return this.m_FreeRun;
			}
			set
			{
				this.m_FreeRun = value;
			}
		}
		...
		public bool CanFreeSpawn
		{
			get
			{
				...
				if (this.Running && this.m_ProximityRange == -1 &&
					(this.m_ObjectPropertyName == null || this.m_ObjectPropertyName.Length == 0) &&
					(this.m_MobPropertyName == null || this.m_MobPropertyName.Length == 0 ||
					 this.m_MobTriggerName == null || this.m_MobTriggerName.Length == 0) &&
					!this.m_ExternalTriggering)
					return true;
				else
					return false;
			}
		...
		public SpawnObject[] SpawnObjects
		{
			get
			{
				return (SpawnObject[])this.m_SpawnObjects.ToArray(typeof(SpawnObject));
			}
			set
			{
				if ((value != null) && (value.Length > 0))
				{
					foreach (SpawnObject so in value)
					{
						...
						if (AlreadyInList == false)
						{
							...
							this.m_SpawnObjects.Add(new SpawnObject(so.TypeName, so.ActualMaxCount, so.SubGroup, so.SequentialResetTime, so.SequentialResetTo, so.KillsNeeded,
		...
		public bool IsFull
		{
			get
			{
				int nobj = this.TotalSpawnedObjects;

				return ((nobj >= this.m_Count) || (nobj >= this.TotalSpawnObjectCount));
			}
		}
		...
		public Region SpawnRegion
		{
			...
			set
			{
				...
				if (this.m_Region != null)
				{
					this.m_RegionName = this.m_Region.Name;
				}
				else
				{
		...
		public string RegionName
		{
			...
			set
			{
				...
				{
					if (string.Compare(region.Name, this.m_RegionName, true) == 0)
					{
						this.m_Region = region;
						this.m_RegionName = region.Name;
						//InvalidateProperties();
						return;
					}
				}
				this.status_str = "invalid region: " + value;
				this.m_Region = null;
			}
		}
		...
		public Point3D X1_Y1
		{
			get
			{
				return new Point3D(this.m_X, this.m_Y, this.Z);
			}
			set
			{
		...
		public int SpawnRange
		{
			...
			set
			{
				...
				this.m_Width = this.m_SpawnRange * 2;
				this.m_Height = this.m_SpawnRange * 2;
				...
				this.m_X = this.Location.X - this.m_SpawnRange;
				this.m_Y = this.Location.Y - this.m_SpawnRange;
		...
		public bool ShowBounds
		{
			get
			{
				return (this.m_ShowBoundsItems != null && this.m_ShowBoundsItems.Count > 0);
			}
			set
			{
				if ((value == true) && (this.ShowBounds == false))
				{
					if (this.m_ShowBoundsItems == null)
						this.m_ShowBoundsItems = new ArrayList();
				...
				if (value == false && this.m_ShowBoundsItems != null)
				{
		...
		public Point3D X2_Y2
		{
			get
			{
				return new Point3D((this.m_X + this.m_Width), (this.m_Y + this.m_Height), this.Z);
			}
			set
			{
				int X2;
		...
		*/
		...
		public int CurrentCount
		{
			get
			{
				return this.TotalSpawnedObjects;
			}
		}
		...
		public bool ExternalTriggering
		{
			get
			{
				return this.m_ExternalTriggering;
			}
			set
			{
				this.m_ExternalTriggering = value;
			}
		}
		...
		public bool ExtTrigState
		{
			get
			{
				return this.m_ExternalTrigger;
			}
			set
			{
				this.m_ExternalTrigger = value;
			}
		}
		...
		public bool HomeRangeIsRelative
		{
			get
			{
				return this.m_HomeRangeIsRelative;
			}
			set
			{
				this.m_HomeRangeIsRelative = value;
			}
		}
		...
		public int KillReset
		{
			get
			{
				return this.m_KillReset;
			}
			set
			{
				this.m_KillReset = value;
			}
		}
		...
		public double TriggerProbability
		{
			get
			{
				return this.m_TriggerProbability;
			}
			set
			{
				this.m_TriggerProbability = value;
			}
		}
		...
		public TimeSpan RefractMin
		{
			get
			{
				return this.m_MinRefractory;
			}
			set
			{
				this.m_MinRefractory = value;
			}
		}
		...
		public TimeSpan RefractMax
		{
			get
			{
				return this.m_MaxRefractory;
			}
			set
			{
				this.m_MaxRefractory = value;
			}
		}
		...
		public TimeSpan RefractoryOver
		{
			get
			{
				if (this.m_refractActivated)
					return this.m_RefractEnd - DateTime.Now;
				else
		...
		public string TriggerObjectProp
		{
			get
			{
				return this.m_ObjectPropertyName;
			}
			set
			{
				this.m_ObjectPropertyName = value;
			}
		}
		...
		public string TriggerObjectName
		{
			get
			{
				if (this.m_ObjectPropertyItem == null || this.m_ObjectPropertyItem.Deleted)
					return null;
				return this.m_ObjectPropertyItem.Name;
			}
		}
		...
		public Item TriggerObject
		{
			get
			{
				return this.m_ObjectPropertyItem;
			}
			set
			{
				this.m_ObjectPropertyItem = value;
			}
		}
		...
		public string SetItemName
		{
			get
			{
				if (this.m_SetPropertyItem == null || this.m_SetPropertyItem.Deleted)
					return null;
				return this.m_SetPropertyItem.Name;
			}
		}
		...
		public Item SetItem
		{
			get
			{
				return this.m_SetPropertyItem;
			}
			set
			{
				this.m_SetPropertyItem = value;
			}
		}
		...
		public string MobTriggerProp
		{
			get
			{
				return this.m_MobPropertyName;
			}
			set
			{
				this.m_MobPropertyName = value;
			}
		}
		...
		public string MobTriggerName
		{
			get
			{
				return this.m_MobTriggerName;
			}
			set
			{
				this.m_MobTriggerName = value;
			}
		}
		...
		public Mobile MobTriggerId
		{
			get
			{
				if (this.m_MobTriggerName == null)
					return null;

				// try to parse out the type information if it has also been saved
				string[] typeargs = this.m_MobTriggerName.Split(",".ToCharArray(), 2);
				string typestr = null;
				string namestr = this.m_MobTriggerName;
		...
		public string PlayerTriggerProp
		{
			get
			{
				return this.m_PlayerPropertyName;
			}
			set
			{
				this.m_PlayerPropertyName = value;
			}
		}
		...
		public TimeSpan TODStart
		{
			get
			{
				return this.m_TODStart;
			}
			set
			{
				this.m_TODStart = value;
			}
		}
		...
		public TimeSpan TOD
		{
			get
			{
				if (this.m_TODMode == TODModeType.Gametime)
				{
					int hours;
					int minutes;
					Server.Items.Clock.GetTime(this.Map, this.Location.X, this.Location.Y, out hours, out minutes);
		...
		public TODModeType TODMode
		{
			get
			{
				return this.m_TODMode;
			}
			set
			{
				this.m_TODMode = value;
			}
		}
		...
		public bool TODInRange
		{
			get
			{
				if (this.m_TODStart == this.m_TODEnd)
					return true;
				DateTime now;
				DateTime TOD_start;
				DateTime TOD_end;
				...
				TOD_start = day_start + this.m_TODStart;
				TOD_end = day_start + this.m_TODEnd;
		...
		public TimeSpan DespawnTime
		{
			get
			{
				return this.m_DespawnTime;
			}
			set
			{
				this.m_DespawnTime = value;
			}
		}
		...
		public TimeSpan DurationOver
		{
			get
			{
				if (this.m_durActivated)
					return this.m_DurEnd - DateTime.Now;
				else
					return TimeSpan.FromSeconds(0);
			}
			set
		...
		public bool ProximityActivated
		{
			get
			{
				return this.m_proximityActivated;
			}
			set
			{
				...
				this.m_proximityActivated = value;
			}
		}
		...
		public int ProximitySound
		{
			get
			{
				return this.m_ProximityTriggerSound;
			}
			set
			{
				this.m_ProximityTriggerSound = value;
			}
		}
		...
		public string ProximityMsg
		{
			get
			{
				return this.m_ProximityTriggerMessage;
			}
			set
			{
				this.m_ProximityTriggerMessage = value;
			}
		}
		...
		public string SpeechTrigger
		{
			get
			{
				return this.m_SpeechTrigger;
			}
			set
			{
				this.m_SpeechTrigger = value;
			}
		}
		...
		public string SkillTrigger
		{
			...
			set
			{
				...
				if (this.m_SkillTrigger != null)
				{
					XmlSpawnerSkillCheck.UnRegisterSkillTrigger(this, this.m_SkillTriggerName, this.Map, false);
				}
				...
				if (news != XmlSpawnerSkillCheck.RegisteredSkill.Invalid)
				{
					...
					this.m_SkillTrigger = value;
					this.m_SkillTriggerName = news;
					this.m_SkillTriggerMin = minval;
					this.m_SkillTriggerMax = maxval;
					this.m_SkillTriggerSuccess = successval;
				}
				else
				{
					this.m_SkillTrigger = null;
					this.m_SkillTriggerName = XmlSpawnerSkillCheck.RegisteredSkill.Invalid;
					this.m_SkillTriggerMin = -1;
					this.m_SkillTriggerMax = -1;
				}
			}
		}
		...
		public TimeSpan NextSpawn
		{
			...
			{
				if (this.m_Running)
					return this.m_End - DateTime.Now;
				else
					return TimeSpan.FromSeconds(0);
			}
			set
		...
		public bool SpawnOnTrigger
		{
			get
			{
				return this.m_SpawnOnTrigger;
			}
			set
			{
				this.m_SpawnOnTrigger = value;
			}
		}
		...
		public string GumpState
		{
			get
			{
				return this.m_GumpState;
			}
			set
			{
				this.m_GumpState = value;
			}
		}
		...
		public int SequentialSpawn
		{
			get
			{
				return this.m_SequentialSpawning;
			}
			set
			{
				this.m_SequentialSpawning = value;
			}
		}
		...
		public TimeSpan NextSeqReset
		{
			get
			{
				if (this.m_Running && ((this.m_SeqEnd - DateTime.Now) > TimeSpan.Zero))
					return this.m_SeqEnd - DateTime.Now;
				else
					return TimeSpan.FromSeconds(0);
			}
			set
			{
				this.m_SeqEnd = DateTime.Now + value;
			}
		}
		...
		public bool AllowNPCTrig
		{
			get
			{
				return this.m_AllowNPCTriggering;
			}
			set
			{
				this.m_AllowNPCTriggering = value;
			}
		}
		...
		public Mobile TriggerMob
		{
			get
			{
				return this.m_mob_who_triggered;
			}
			set
			{
				this.m_mob_who_triggered = value;
			}
		}
		...
		public string FirstModifiedBy
		{
			get
			{
				return this.m_FirstModifiedBy;
			}
			set
			{
				this.m_FirstModifiedBy = value;
				this.m_FirstModified = DateTime.Now;
			}
		}
		...
		public string LastModifiedBy
		{
			...
			set
			{
				this.m_LastModifiedBy = value;
				this.m_LastModified = DateTime.Now;
			}
		}
		...
		public bool SmartSpawning
		{
			get
			{
				return this.m_SmartSpawning;
			}
			set
			{
				this.m_SmartSpawning = value;

				if (this.m_SmartSpawning)
				{
		...
		public override void OnMapChange()
		{
			...
			XmlSpawnerSkillCheck.UnRegisterSkillTrigger(this, this.m_SkillTriggerName, this.currentmap, false);

			// register the skill trigger on the new current map
			XmlSpawnerSkillCheck.RegisterSkillTrigger(this, this.m_SkillTriggerName, this.Map);
		...
		{
			if (from == null || from.Deleted || from.AccessLevel < AccessLevel.Spawner || (this.m_SpawnerGump != null && this.SomeOneHasGumpOpen))
				return;

			// flag the first person to open the spawner as the placer
			if (this.FirstModifiedBy == null)
				this.FirstModifiedBy = from.Name;

			this.LastModifiedBy = from.Name;

			// clear any text entry books that might still be around
			this.DeleteTextEntryBook();
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			list.Add(1042971, " " + this.Name); // ~1_val~
			...
			if (this.m_Group)
			{
				list.Add(1060658 + 6 - nlist_items, "group\t{0}", this.m_Group); // ~1_val~: ~2_val~
				nlist_items--;
			}

			if (this.m_Team != 0)
			{
				list.Add(1060658 + 6 - nlist_items, "team\t{0}", this.m_Team); // ~1_val~: ~2_val~
				nlist_items--;
			}

			list.Add(1060658 + 6 - nlist_items, "speed\t{0} to {1}", this.m_MinDelay, this.m_MaxDelay); // ~1_val~: ~2_val~
			nlist_items--;

			// display the duration parameter in the prop gump if it is non-zero
			if (this.m_Duration > TimeSpan.FromMinutes(0))
			{
				list.Add(1060658 + 6 - nlist_items, "Duration\t{0}", this.m_Duration);
				nlist_items--;
			}

			// display the proximity range parameter in the prop gump if it is active
			if (this.m_ProximityRange != -1)
			{
				list.Add(1060658 + 6 - nlist_items, "ProximityRange\t{0}", this.m_ProximityRange);
				nlist_items--;
			}

			if (this.m_SpawnObjects != null)
				for (int i = 0; i < nlist_items && i < this.m_SpawnObjects.Count; ++i)
				{
					string typename = ((SpawnObject)this.m_SpawnObjects[i]).TypeName;
					if (typename != null && typename.Length > 20)
					{
						typename = typename.Substring(0, 20);
					}

					list.Add(1060658 + (6 - nlist_items) + i, " {0}\t{1}", typename, ((SpawnObject)this.m_SpawnObjects[i]).SpawnedObjects.Count);
				}
		...
		{
			this.LabelTo(from, "XmlSpawner");
			this.LabelTo(from, this.Name + (this.m_Running == true ? " [On]" : " [Off]"));
		}
		...
		public override void OnDelete()
		{
			...
			if (this.m_DurTimer != null)
				this.m_DurTimer.Stop();

			if (this.m_RefractoryTimer != null)
				this.m_RefractoryTimer.Stop();

			// if statics were added for marking container held spawners, delete them
			if (this.m_ShowContainerStatic != null && !this.m_ShowContainerStatic.Deleted)
				this.m_ShowContainerStatic.Delete();
		...
		private void ResetSectorList()
		{
			...
			bool sectorrefresh = this.HasActiveSectors;
		}
		...
		public void LoadXmlConfig(string filename)
		{
			...
			if (System.IO.File.Exists(filename) == true)
			{
				...
				if (fs == null)
				{
					this.status_str = String.Format("Unable to open {0} for loading", filename);
					return;
				}
				...
				if (ds.Tables != null && ds.Tables.Count > 0)
				{
					if (ds.Tables[XmlTableName] != null && ds.Tables[XmlTableName].Rows.Count > 0)
					{
						foreach (DataRow dr in ds.Tables[XmlTableName].Rows)
						{
							...
							if (valid_entry)
							{
								this.m_SequentialSpawning = intEntry;
							}
							...
							if (valid_entry)
							{
								this.m_ProximityRange = intEntry;
							}
							...
							if (valid_entry)
							{
								this.m_ProximityTriggerMessage = strEntry;
							}
							...
							if (valid_entry)
							{
								this.m_ProximityTriggerSound = intEntry;
							}
							...
							if (valid_entry)
							{
								this.m_ItemTriggerName = strEntry;
							}
							...
							if (valid_entry)
							{
								this.m_NoItemTriggerName = strEntry;
							}
							...
							{
								if (delayinsec)
									this.m_MinDelay = TimeSpan.FromSeconds(doubleEntry);
								else
									this.m_MinDelay = TimeSpan.FromMinutes(doubleEntry);
							}
							...
							{
								if (delayinsec)
									this.m_MaxDelay = TimeSpan.FromSeconds(doubleEntry);
								else
									this.m_MaxDelay = TimeSpan.FromMinutes(doubleEntry);
							}
							...
							if (valid_entry)
							{
								this.m_Duration = TimeSpan.FromMinutes(doubleEntry);
							}
							...
							if (valid_entry)
							{
								this.m_DespawnTime = TimeSpan.FromHours(doubleEntry);
							}
							...
							if (valid_entry)
							{
								this.m_MinRefractory = TimeSpan.FromMinutes(doubleEntry);
							}
							...
							if (valid_entry)
							{
								this.m_MaxRefractory = TimeSpan.FromMinutes(doubleEntry);
							}
							...
							if (valid_entry)
							{
								this.m_TODStart = TimeSpan.FromMinutes(doubleEntry);
							}
							...
							if (valid_entry)
							{
								this.m_TODEnd = TimeSpan.FromMinutes(doubleEntry);
							}
							...
							if (valid_entry)
							{
								this.m_TODMode = (TODModeType)intEntry;
							}
							...
							if (valid_entry)
							{
								this.m_WayPoint = GetWaypoint(strEntry);
							}
							...
							if (valid_entry)
							{
								this.m_TriggerProbability = doubleEntry;
							}
							...
							if (valid_entry)
							{
								this.m_ExternalTriggering = boolEntry;
							}
							...
							if (valid_entry)
							{
								this.m_HomeRangeIsRelative = boolEntry;
							}
							...
							if (valid_entry)
							{
								this.m_AllowGhostTriggering = boolEntry;
							}
							...
							if (valid_entry)
							{
								this.m_AllowNPCTriggering = boolEntry;
							}
							...
							if (valid_entry)
							{
								this.m_SpawnOnTrigger = boolEntry;
							}
							...
							if (valid_entry)
							{
								this.m_SmartSpawning = boolEntry;
							}
							...
							if (valid_entry)
							{
								this.m_PlayerPropertyName = strEntry;
							}
							...
							if (valid_entry)
							{
								this.m_MobPropertyName = strEntry;
							}
							...
							if (valid_entry)
							{
								this.m_MobTriggerName = strEntry;
							}
							...
							if (valid_entry)
							{
								this.m_ObjectPropertyName = strEntry;
							}
							...
							if (valid_entry)
							{
								...
								this.m_ObjectPropertyItem = BaseXmlSpawner.FindItemByName(this, namestr, typestr);
							}
							...
							if (valid_entry)
							{
								...
								this.m_SetPropertyItem = BaseXmlSpawner.FindItemByName(this, namestr, typestr);
							}
		...
		public void ReportStatus()
		{
			...
			{
				Console.WriteLine("PropertyInfoList: {0}", this.PropertyInfoList.Count);
				foreach (BaseXmlSpawner.TypeInfo to in this.PropertyInfoList)
				{
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
		...
		private bool ValidPlayerTrig(Mobile m)
		{
			if (m == null || m.Deleted)
				return false;
			return ((m.Player || this.m_AllowNPCTriggering) && (m.AccessLevel <= this.TriggerAccessLevel) && ((!m.Body.IsGhost && !this.m_AllowGhostTriggering) || (m.Body.IsGhost && this.m_AllowGhostTriggering)));
		}
		...
		private bool AllowTriggering
		{
			get
			{
				return this.m_Running && !this.m_refractActivated && this.TODInRange && this.CanSpawn;
			}
		}
		...
		private void ActivateTrigger()
		{
			...
			if (this.m_MaxRefractory > TimeSpan.FromMinutes(0))
			{
				int minSeconds = (int)this.m_MinRefractory.TotalSeconds;
				int maxSeconds = (int)this.m_MaxRefractory.TotalSeconds;

				this.DoTimer3(TimeSpan.FromSeconds(Utility.RandomMinMax(minSeconds, maxSeconds)));
			}

			// if the spawnontrigger flag is set, then spawn immediately
			if (this.m_SpawnOnTrigger)
			{
				this.NextSpawn = TimeSpan.Zero;
				this.ResetNextSpawnTimes();
			}

			// reset speech triggering if it was set
			this.m_speechTriggerActivated = false;

			// reset skill triggering if it was set
			this.m_skillTriggerActivated = false;
		...
		public void CheckTriggers(Mobile m, Skill s, bool hasproximity)
		{
			...
			if (this.AllowTriggering && !this.m_proximityActivated)
			{
				...
				if (this.m_ExternalTriggering && !this.m_ExternalTrigger)
					return;

				// if speech triggering is set then test for successful activation
				if (this.m_SpeechTrigger != null && this.m_SpeechTrigger.Length > 0)
				{
					needs_speech_trigger = true;
				}
				// check to see if we have to continue
				if (needs_speech_trigger && !this.m_speechTriggerActivated)
					return;

				// if skill triggering is set then test for successful activation
				if (this.m_SkillTrigger != null && this.m_SkillTrigger.Length > 0)
				{
					needs_skill_trigger = true;
				}
				// check to see if we have to continue
				if (needs_skill_trigger && !this.m_skillTriggerActivated)
					return;

				// if item property triggering is set then test for the property value
				//
				if (this.m_ObjectPropertyName != null && this.m_ObjectPropertyName.Length > 0)
				{
					needs_object_trigger = true;
					string status_str;

					if (BaseXmlSpawner.TestItemProperty(this, this.m_ObjectPropertyItem, this.m_ObjectPropertyName, null, out status_str))
					{
				...
				if (this.m_PlayerPropertyName != null && this.m_PlayerPropertyName.Length > 0)
				{
					needs_player_trigger = true;
					string status_str;

					if (BaseXmlSpawner.TestMobProperty(this, m, this.m_PlayerPropertyName, null, out status_str))
					{
				...
				if (this.m_MobPropertyName != null && this.m_MobPropertyName.Length > 0 &&
					this.m_MobTriggerName != null && this.m_MobTriggerName.Length > 0)
				{
					...
					if (BaseXmlSpawner.TestMobProperty(this, this.MobTriggerId, this.m_MobPropertyName, null, out status_str))
					{
				...
				if (this.m_ItemTriggerName != null && this.m_ItemTriggerName.Length > 0)
				{
					//enable_triggering = false;
					needs_item_trigger = true;

					has_item_trigger = BaseXmlSpawner.CheckForCarried(m, this.m_ItemTriggerName);
				}
				// check to see if we have to continue
				...
				if (this.m_NoItemTriggerName != null && this.m_NoItemTriggerName.Length > 0)
				{
					needs_noitem_trigger = true;

					has_noitem_trigger = BaseXmlSpawner.CheckForNotCarried(m, this.m_NoItemTriggerName);
				}
				// check to see if we have to continue
				...
				if (!hasproximity && !needs_object_trigger && !needs_mob_trigger && !this.m_ExternalTriggering)
					return;

				// all of the necessary trigger conditions have been met so go ahead and trigger
				// after you make the probability check

				if (Utility.RandomDouble() < this.m_TriggerProbability)
				{
					// play a sound indicating the spawner has been triggered
					if (this.m_ProximityTriggerSound > 0 && m != null && !m.Deleted)
						m.PlaySound(this.m_ProximityTriggerSound);

					// display the trigger message
					if (this.m_ProximityTriggerMessage != null && this.m_ProximityTriggerMessage.Length > 0 && m != null && !m.Deleted)
						m.PublicOverheadMessage(MessageType.Regular, 0x3B2, true, this.m_ProximityTriggerMessage);

					// enable spawning at the next ontick
					// this will also start the refractory timer and send the triggering indicators
					this.ProximityActivated = true;

					// keep track of who triggered this
					this.m_mob_who_triggered = m;

					// keep track of the skill that triggered this
					if (s != null)
					{
						this.m_skill_that_triggered = s.SkillName;
					}
					else
					{
						this.m_skill_that_triggered = XmlSpawnerSkillCheck.RegisteredSkill.Invalid;
		...
		public bool HandlesOnSkillUse
		{
			get
			{
				return (this.m_Running && this.m_SkillTrigger != null && this.m_SkillTrigger.Length > 0);
			}
		}
		...
		{
			if (this.m_Running && this.m_ProximityRange >= 0 && this.ValidPlayerTrig(m) && this.CanSpawn && !this.m_refractActivated && this.TODInRange)
			{
				if (!Utility.InRange(m.Location, this.Location, this.m_ProximityRange))
					return;

				this.m_skillTriggerActivated = false;

				// check the skill trigger conditions, Skillname[+/-][,min,max]
				if (this.m_SkillTrigger != null && (skill.SkillName == this.m_SkillTriggerName) &&
					((this.m_SkillTriggerMin < 0) || (skill.Value >= this.m_SkillTriggerMin)) &&
					((this.m_SkillTriggerMax < 0) || (skill.Value <= this.m_SkillTriggerMax)) &&
					((this.m_SkillTriggerSuccess == 3) || ((this.m_SkillTriggerSuccess == 1) && success) || ((this.m_SkillTriggerSuccess == 2) && !success)))
				{
					// have a skill trigger so flag it and test it
					this.m_skillTriggerActivated = true;

					this.CheckTriggers(m, skill, true);
				}
			}
		}
		...
		public override bool HandlesOnSpeech
		{
			get
			{
				return (this.m_Running && this.m_SpeechTrigger != null && this.m_SpeechTrigger.Length > 0);
			}
		}

		public override void OnSpeech(SpeechEventArgs e)
		{
			if (/*!e.Handled && */this.m_Running && this.m_ProximityRange >= 0 && this.ValidPlayerTrig(e.Mobile) && this.CanSpawn && !this.m_refractActivated && this.TODInRange)
			{
				this.m_speechTriggerActivated = false;

				if (!Utility.InRange(e.Mobile.Location, this.Location, this.m_ProximityRange))
					return;

				if (this.m_SpeechTrigger != null && e.Speech.ToLower().IndexOf(this.m_SpeechTrigger.ToLower()) >= 0)
				{
					e.Handled = true;

					// found the speech trigger so flag it for testing in the onmovement handler where the other proximity features are tested
					this.m_speechTriggerActivated = true;

					this.CheckTriggers(e.Mobile, null, true);
				}
			}
		}
		...
		public override bool HandlesOnMovement
		{
			get
			{
				return (this.m_Running && this.m_ProximityRange >= 0);
			}
		}
		...
		public void AddToMovementList(Mobile m)
		{
			...
			if (this.m_MovementList == null)
			{
				this.m_MovementList = new ArrayList();
			}

			// check to see if the movement timer is running
			if (this.m_MovementTimer == null || !this.m_MovementTimer.Running)
			{
				this.DoMovementTimer(TimeSpan.FromSeconds(1));
			}

			bool add = true;

			foreach (MovementInfo moveinfo in this.m_MovementList)
			{
			...
			if (add)
			{
				...
				if (this.m_MovementList.Count > MaxMoveCheck)
				{
					// replace a random entry in the current list with this one
					this.m_MovementList[Utility.Random(this.m_MovementList.Count)] = new MovementInfo(m);
				}
				else
				{
					this.m_MovementList.Add(new MovementInfo(m));
				}
			}
		}
		...
		{
			if (this.m_MovementTimer != null)
				this.m_MovementTimer.Stop();

			this.m_MovementTimer = new MovementTimer(this, delay);

			this.m_MovementTimer.Start();
		}
		...
		private class MovementTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				{
					if (this.m_Spawner.m_Running && !this.m_Spawner.m_proximityActivated && !this.m_Spawner.m_refractActivated && this.m_Spawner.TODInRange && this.m_Spawner.CanSpawn)
					{
						int count = 0;
						int maxspeed = 0;
						int speed = 0;
						foreach (MovementInfo moveinfo in this.m_Spawner.m_MovementList)
						{
							...
							if (speed > maxspeed)
								maxspeed = speed;
							this.m_Spawner.CheckTriggers(m, null, true);
						}

						this.m_Spawner.MovingPlayerCount = this.m_Spawner.m_MovementList.Count;
						this.m_Spawner.FastestPlayerSpeed = maxspeed;
					}
					this.m_Spawner.m_MovementList.Clear();
				}
			}
		}
		...
		{
			if (this.m_Running && this.m_ProximityRange >= 0 && this.ValidPlayerTrig(m) && this.CanSpawn)
			{
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register("XmlHome", AccessLevel.Spawner, new CommandEventHandler(XmlHome_OnCommand));
			...
			CommandSystem.Register("XmlGet", AccessLevel.Spawner, new CommandEventHandler(XmlGetValue_OnCommand));
			...
			CommandSystem.Register("SmartStat", AccessLevel.Spawner, new CommandEventHandler(SmartStat_OnCommand));
			CommandSystem.Register("XmlGo", AccessLevel.Spawner, new CommandEventHandler(SpawnEditorGo_OnCommand));
		...
		private class GetValueTarget : Target
		{
			...
			{
				string pname = this.m_e.GetString(0);
				Type ptype;
		...
		public class XmlSetCommand : BaseCommand
		{
			...
			{
				this.AccessLevel = AccessLevel.Spawner;
				this.Supports = CommandSupport.All;
				this.Commands = new string[] { "XmlSet" };
				this.ObjectTypes = ObjectTypes.Both;
				this.Usage = "XmlSet <propertyName> <value>";
				this.Description = "Sets a property value by name of a targeted object. Provides access to all public properties.";
			}
			...
			public override void Execute(CommandEventArgs e, object obj)
			{
				...
				{
					this.LogFailure("Format: XmlSet <propertyName> <value>");
				}
			}
		}
		...
		private class XmlHomeTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is XmlSpawner)
				{
					if (this.m_e.GetString(0) == "status")
					{
				...
				foreach (SpawnObject so in spawner.m_SpawnObjects)
				{
					for (int x = 0; x < so.SpawnedObjects.Count; x++)
					{
						...
						if (o == targeted)
						{
							...
							else if (this.m_e.GetString(0) == "send")
							{
								// make sure the spawner is not in a container.
								if (spawner.Parent == null)
								{
							...
							else if (this.m_e.GetString(0) == "gump")
							{
		...
		private static void LoadDefaults(XmlElement node)
		{
			try
			{
				XmlSpawner.defProximityRange = int.Parse(node["defProximityRange"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defTriggerProbability = double.Parse(node["defTriggerProbability"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defProximityTriggerSound = int.Parse(node["defProximityTriggerSound"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defMinRefractory = TimeSpan.Parse(node["defMinRefractory"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defMaxRefractory = TimeSpan.Parse(node["defMaxRefractory"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defTODStart = TimeSpan.Parse(node["defTODStart"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defTODEnd = TimeSpan.Parse(node["defTODEnd"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defAmount = int.Parse(node["defStackAmount"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defDuration = TimeSpan.Parse(node["defDuration"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defIsGroup = bool.Parse(node["defIsGroup"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defTeam = int.Parse(node["defTeam"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defRelativeHome = bool.Parse(node["defRelativeHome"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defSpawnRange = int.Parse(node["defSpawnRange"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defHomeRange = int.Parse(node["defHomeRange"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defMinDelay = TimeSpan.Parse(node["defMinDelay"].InnerText);
			}
			catch
			{
			}
			try
			{
				XmlSpawner.defMaxDelay = TimeSpan.Parse(node["defMaxDelay"].InnerText);
			}
			catch
			{
			}
		...
		public static void XmlUnLoadFromFile(string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
		{
			...
			else if (System.IO.Directory.Exists(filename) == true)
			{
			...
			: base(BaseItemId)
		{
			...
			this.SpawnRange = defSpawnRange;

			this.InitSpawn(0, 0, this.m_Width, this.m_Height, string.Empty, 0, defMinDelay, defMaxDelay, defDuration,
			...
			: base(BaseItemId)
		{
			this.m_PlayerCreated = true;
			this.m_UniqueId = Guid.NewGuid().ToString();
			this.SpawnRange = homeRange;
			SpawnObject[] so = new SpawnObject[1];
			so[0] = new SpawnObject(creatureName, amount);

			this.InitSpawn(0, 0, this.m_Width, this.m_Height, string.Empty, amount, TimeSpan.FromMinutes(minDelay), TimeSpan.FromMinutes(maxDelay), defDuration,
			...
			: base(BaseItemId)
		{
			...
			this.InitSpawn(0, 0, this.m_Width, this.m_Height, string.Empty, 1, defMinDelay, defMaxDelay, defDuration,
			...
			: base(BaseItemId)
		{
			...
			this.InitSpawn(x, y, width, height, name, maxCount, minDelay, maxDelay, duration,
			...
			string configfile, TimeSpan despawnTime, string skillTrigger, bool smartSpawning, WayPoint wayPoint)
		{
			...
			if (width == height)
				this.m_SpawnRange = width / 2;
			else
				this.m_SpawnRange = -1;
			this.m_Running = true;
			this.m_Group = isGroup;
			...
			this.m_MinRefractory = minRefractory;
			this.m_MaxRefractory = maxRefractory;
			this.m_TODStart = todstart;
			this.m_TODEnd = todend;
			this.m_TODMode = todMode;
			this.m_KillReset = killReset;
			this.m_Duration = duration;
			this.m_DespawnTime = despawnTime;
			this.m_ProximityRange = proximityRange;
			this.m_ProximityTriggerSound = proximityTriggerSound;
			this.m_proximityActivated = false;
			this.m_durActivated = false;
			this.m_refractActivated = false;
			this.m_Count = maxCount;
			this.m_Team = team;
			this.m_StackAmount = amount;
			this.m_HomeRange = homeRange;
			this.m_HomeRangeIsRelative = isRelativeHomeRange;
			this.m_ObjectPropertyItem = objectPropertyItem;
			this.m_ObjectPropertyName = objectPropertyName;
			this.m_ProximityTriggerMessage = proximityMessage;
			this.m_ItemTriggerName = itemTriggerName;
			this.m_NoItemTriggerName = noitemTriggerName;
			this.m_SpeechTrigger = speechTrigger;
			this.SkillTrigger = skillTrigger;        // note this will register the skill as well
			this.m_MobTriggerName = mobTriggerName;
			this.m_MobPropertyName = mobPropertyName;
			this.m_PlayerPropertyName = playerPropertyName;
			this.m_TriggerProbability = triggerProbability;
			this.m_SetPropertyItem = setPropertyItem;
			this.m_ExternalTriggering = externalTriggering;
			this.m_ExternalTrigger = false;
			this.m_SequentialSpawning = sequentialSpawning;
			this.RegionName = regionName;
			this.m_AllowGhostTriggering = allowghost;
			this.m_AllowNPCTriggering = allownpc;
			this.m_SpawnOnTrigger = spawnontrigger;
			this.m_SmartSpawning = smartSpawning;
		...
		public void Defrag(bool killtest)
		{
			...
			foreach (SpawnObject so in this.m_SpawnObjects)
			{
				for (int x = 0; x < so.SpawnedObjects.Count; x++)
				{
					...
					if (o is Item)
					{
						...
						if (this.DespawnTime.TotalHours > 0 && !item.Deleted && (item.LastMoved < DateTime.Now - this.DespawnTime) && (item.Parent == this.Parent) &&
							...
							(ItemFlags.GetTaken(item) && (item.Parent == null || (item.Parent != this.Parent))))   // taken and in the world, or a different container
						{
							...
							if (this.SequentialSpawn >= 0 && so.RestrictKillsToSubgroup)
							{
								if (so.SubGroup == this.SequentialSpawn)
									total_removed++;
							}
							else
							{
					...
					else if (o is Mobile)
					{
						...
						* */
						...
						if (this.DespawnTime.TotalHours > 0 && !m.Deleted && (m.CreationTime < DateTime.Now - this.DespawnTime) &&
							m.Map != null && m.Map != Map.Internal && !m.Map.GetSector(m.Location).Active)
						{
							//m.Delete();
							deletelist.Add(m);
			...
			this.DeleteFromList(deletelist);
			...
			if (killtest)
				this.m_killcount += total_removed;
		}
		...
		private int SubGroupCount(int sgroup)
		{
			...
			{
				SpawnObject s = (SpawnObject)this.m_SpawnObjects[i];
		...
		private int RandomAvailableSpawnIndex()
		{
			...
			return (this.RandomAvailableSpawnIndex(-1));
		}
		...
		private int RandomAvailableSpawnIndex(int sgroup)
		{
			...
			for (int i = 0; i < this.m_SpawnObjects.Count; i++)
			{
				...
				if ((s.MaxCount > s.SpawnedObjects.Count) && (sgroup < 0 || sgroup == s.SubGroup) &&
					(sgrouplist == null || !sgrouplist.Contains(s.SubGroup)) && (s.SubGroup <= 0 || this.SubGroupCount(s.SubGroup) + totalcount <= this.MaxCount))
				{
		...
		{
			if (this.m_SpawnObjects == null || this.m_SpawnObjects.Count == 0)
				return (0);
			...
			for (int j = 0; j < this.m_SpawnObjects.Count; j++)
			{
				SpawnObject s = (SpawnObject)this.m_SpawnObjects[j];
				if (s.SubGroup > 0 && (s.Ignore || s.Disabled))
					continue;
			...
			if (largergroup < 0 && sgroup >= 0)
				return (this.NextSequentialIndex(-1));
			else
				return (largergroup);
		}
		...
		public int GetCurrentAvailableSequentialSpawnIndex(int sgroup)
		{
			...
			if (sgroup == 0)
				return (this.RandomAvailableSpawnIndex(0));
		...
		public int GetCurrentSequentialSpawnIndex(int sgroup)
		{
			...
			if (sgroup == 0)
				return (this.RandomSpawnIndex(0));

			//return the first instance of a spawn object that is an available member of the requested subgroup
			for (int j = 0; j < this.m_SpawnObjects.Count; j++)
			{
				if (((SpawnObject)this.m_SpawnObjects[j]).SubGroup == sgroup)
				{
					return (j);
				}
			}
		...
		private void SeqResetTo(int sgroup)
		{
			...
			int spawnindex = this.GetCurrentSequentialSpawnIndex(sgroup);

			if (spawnindex >= 0)
			{
				// if it is greater than zero then initiate reset
				SpawnObject s = (SpawnObject)this.m_SpawnObjects[spawnindex];
				this.m_SequentialSpawning = s.SequentialResetTo;

				this.InitiateSequentialReset(sgroup);

				// clear the spawns
				//RemoveSpawnObjects();
				this.ClearSubgroup(s.SubGroup);
		...
		private bool CheckForSequentialReset()
		{
			...
			// cant do resets on subgroup 0
			if (this.m_SequentialSpawning == 0)
				return false;

			// this will get the index of the first spawn entry in the subgroup
			// it will have the subgroup timer settings
			int spawnindex = this.GetCurrentSequentialSpawnIndex(this.m_SequentialSpawning);
			...
			if (spawnindex >= 0)
			{
				...
				if (s.SequentialResetTime > 0)
				{
					...
					if (this.NextSeqReset <= TimeSpan.Zero)
					{
		...
		private void InitiateSequentialReset(int sgroup)
		{
			...
			if (spawnindex >= 0)
			{
				// if it is greater than zero then initiate reset
				SpawnObject s = (SpawnObject)this.m_SpawnObjects[spawnindex];
				this.NextSeqReset = TimeSpan.FromMinutes(s.SequentialResetTime);
			}
		}
		...
		public void ResetSequential()
		{
			...
			if (this.m_SequentialSpawning >= 0)
			{
				this.m_SequentialSpawning = this.NextSequentialIndex(-1);
			}
		...
		public bool AdvanceSequential()
		{
			...
			if (this.HoldSequence)
				return false;

			// check for triggering
			if (!((this.m_proximityActivated || this.CanFreeSpawn) && this.TODInRange))
				return false;
			...
			if (this.m_SequentialSpawning >= 0 && (killsneeded == 0 || this.KillCount >= killsneeded))
			{
				this.m_SequentialSpawning = this.NextSequentialIndex(this.m_SequentialSpawning);
				...
				this.InitiateSequentialReset(this.m_SequentialSpawning);

				// clear the spawns if there is a killcount on the level
				if (killsneeded >= 0)
				{
					this.ClearSubgroup(subgroup);
					clearedobjects = true;
				}
		...
		public void OnTick()
		{
			...
			int startcount = this.killcount_held;
			if (!this.m_skipped)
			{
				this.killcount_held = this.m_killcount;
			}
			...
			if (startcount == this.m_killcount && !this.m_refractActivated && !this.m_skipped)
			{
				this.m_spawncheck--;
			}
			this.m_skipped = false;
			...
			if (this.m_spawncheck <= 0)
			{
				this.m_killcount = 0;
				this.m_spawncheck = this.m_KillReset;     // wait for 1 spawn ticks to pass before resetting.  This can be set to anything you like
			}

			// check for smart spawning
			if (this.SmartSpawning && this.IsFull && !this.HasActiveSectors && !this.HasDamagedOrDistantSpawns /*&& !HasHoldSmartSpawning */)
			{
			...
			if (!this.OnHold)
			{
				...
				if (this.m_ProximityRange == -1 && this.CanSpawn)
				{
					this.CheckTriggers((Mobile)null, null, false);
				}

				// check for proximity triggers without movement activation
				if (this.m_ProximityRange >= 0 && this.CanSpawn)
				{
					// check all nearby players
					foreach (Mobile p in this.GetMobilesInRange(this.m_ProximityRange))
					{
						if (this.ValidPlayerTrig(p))
							this.CheckTriggers(p, null, true);
					}
				}
				...
				if (this.m_Group == true)
				{
					...
					if (this.CheckForSequentialReset())
					{
						// it has expired so reset the sequential spawn level
						this.SeqResetTo(this.m_SequentialSpawning);

						bool triedtospawn = this.Respawn();
						...
						if (!triedtospawn)
							this.HoldSequence = true;
					}
					else if (this.TotalSpawnedObjects <= 0)
					{
				...
				else
				{
					...
					bool triedtospawn = this.Spawn(false);
				...
				if (!this.OnHold && !this.FreeRun)
				{
					this.m_proximityActivated = false;
				}
			}

			if (this.FreeRun && this.SpawnOnTrigger && this.m_proximityActivated)
			{
		...
		public bool ClearSpawnedThisTick
		{
			set
			{
				if (this.m_SpawnObjects == null || value == false)
					return;

				for (int i = 0; i < this.m_SpawnObjects.Count; i++)
				{
					SpawnObject sobj = (SpawnObject)this.m_SpawnObjects[i];
					if (sobj != null)
					{
		...
		{
			if (this.m_SpawnObjects != null && this.m_SpawnObjects.Count > 0 && (this.m_proximityActivated || this.CanFreeSpawn) && this.TODInRange)
			{
				this.m_HoldSequence = false;

				// if the spawner is full then dont bother
				if (this.IsFull)
				{
					this.ResetProximityActivated();
					return (true);
				}
				...
				if (this.m_SequentialSpawning >= 0)
				{
					...
					SpawnIndex = this.GetCurrentAvailableSequentialSpawnIndex(this.m_SequentialSpawning);
				}
				else
				{
					// if sequential spawning is not set then select the next spawn at random
					SpawnIndex = this.RandomAvailableSpawnIndex();
				}
				...
				else
				{
					...
					if (this.Spawn(SpawnIndex, smartspawn, sobj.SpawnsPerTick) == true)
					{
						if (!smartspawn)
							this.RefreshNextSpawnTime(sobj);
					}
				}
		...
		{
			if (this.m_SpawnObjects == null || index >= this.m_SpawnObjects.Count)
				return false;

			bool didspawn = false;

			SpawnObject so = (SpawnObject)this.m_SpawnObjects[index];
			...
			int scnt = this.SafeCurrentCount;

			for (int k = 0; (k < nspawn) && (k + socnt < somax) && (k + scnt < this.MaxCount); k++)
			{
				if (packrange >= 0 && so.SubGroup > 0 && packcoord == Point3D.Zero)
				{
					packcoord = this.GetPackCoord(so.SubGroup);
				}
				if (this.Spawn(index, smartspawn, packrange, packcoord, ignoreloopprotection))
				{
		...
		{
			return this.Spawn(index, smartspawn, count, false);
		}

		// spawn an individual entry by index up to count times
		public bool Spawn(int index, bool smartspawn, int count, bool ignoreloopprotection)
		{
			return this.Spawn(index, smartspawn, count, -1, Point3D.Zero, ignoreloopprotection);
		}
		...
		public void Spawn(string SpawnObjectTypeName, bool smartspawn, int packrange, Point3D packcoord)
		{
			...
			for (int i = 0; i < this.m_SpawnObjects.Count; i++)
			{
				if (((SpawnObject)this.m_SpawnObjects[i]).TypeName.ToUpper() == SpawnObjectTypeName.ToUpper())
				{
					if (this.Spawn(i, smartspawn, packrange, packcoord))
					{
						this.RefreshNextSpawnTime((SpawnObject)this.m_SpawnObjects[i]);
					}
					break;
				}
				else
		...
		{
			this.Spawn(SpawnObjectTypeName, smartspawn, -1, Point3D.Zero);
		}

		// spawn an individual entry by index
		public bool Spawn(int index, bool smartspawn, int packrange, Point3D packcoord)
		{
			return this.Spawn(index, smartspawn, packrange, packcoord, false);
		}
		...
		public bool Spawn(int index, bool smartspawn, int packrange, Point3D packcoord, bool ignoreloopprotection)
		{
			...
			if (TheSpawn != null)
			{
				...
				string substitutedtypeName = BaseXmlSpawner.ApplySubstitution(this, this, this.m_mob_who_triggered, TheSpawn.TypeName);
				...
				while (substitutedtypeName.StartsWith("#"))
				{
					...
					if (args.Length > 0)
					{
						...
						if (keyvalueargs.Length > 0)
						{
							...
							{
								case "#NOTILES":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.NoTiles, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#TILES":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.Tiles, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#WET":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.Wet, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#XFILL":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.RowFill, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#YFILL":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.ColFill, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#EDGE":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.Perimeter, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#PLAYER":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.Player, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#WAYPOINT":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.Waypoint, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#RELXY":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.RelXY, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#DXY":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.DeltaLocation, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#XY":
									spawnpositioning.Add(new SpawnPositionInfo(SpawnPositionType.Location, this.m_mob_who_triggered, keyvalueargs));
									break;
								case "#CONDITION":
									...
									if (ckeyvalueargs.Length > 1)
									{
										...
										if (!BaseXmlSpawner.CheckPropertyString(this, this, ckeyvalueargs[1], this.m_mob_who_triggered, out this.status_str))
											return false;
									}
									else
									{
				...
				if (BaseXmlSpawner.IsTypeOrItemKeyword(typeName))
				{
					...
					bool completedtypespawn = BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName, substitutedtypeName, requiresurface, spawnpositioning,
						this.m_mob_who_triggered, this.Location, this.Map, new XmlGumpCallback(SpawnerGumpCallback), out status_str);
					...
					if (type != null && !(this.Parent != null && (type == typeof(Mobile) || type.IsSubclassOf(typeof(Mobile)))))
					{
						...
						try
						{
							if (o is Mobile)
							{
								...
								*/
								loc = this.GetSpawnPosition(requiresurface, packrange, packcoord, spawnpositioning, m);
								...
								{
									BaseCreature c = (BaseCreature)m;
									c.RangeHome = this.m_HomeRange;
									c.CurrentWayPoint = this.m_WayPoint;
								...
								BaseXmlSpawner.ApplyObjectStringProperties(this, substitutedtypeName, m, this.m_mob_who_triggered, this, out status_str);
							...
							else if (o is Item)
							{
								...
								BaseXmlSpawner.AddSpawnItem(this, TheSpawn, item, this.Location, map, this.m_mob_who_triggered, requiresurface, spawnpositioning, substitutedtypeName, smartspawn, out status_str);
		...
		{
			return this.SpawnSubGroup(sgroup, false);
		}

		public bool SpawnSubGroup(int sgroup, bool smartspawn)
		{
			return this.SpawnSubGroup(sgroup, false, false);
		}
		...
		public bool SpawnSubGroup(int sgroup, bool smartspawn, bool ignoreloopprotection)
		{
			...
			if (sgroup >= 0)
			{
				...
				for (int j = 0; j < this.m_SpawnObjects.Count; j++)
				{
					SpawnObject so = (SpawnObject)this.m_SpawnObjects[j];
					...
					if (so != null && so.SubGroup == sgroup)
					{
						...
						{
							packcoord = this.GetPackCoord(sgroup);
						}

						// get the SpawnsPerTick count and spawn up to that number
						bool success = this.Spawn(j, smartspawn, so.SpawnsPerTick, so.PackRange, packcoord, ignoreloopprotection);

						if (success)
							didspawn = true;

						if (success && !smartspawn)
							this.RefreshNextSpawnTime(so);
					}
				}
			...
			foreach (SpawnObject so in this.m_SpawnObjects)
			{
				for (int i = 0; i < so.SpawnedObjects.Count; ++i)
				{
					...
					if (o is Mobile)
					{
						...
						m.Map = this.Map;
						m.Location = new Point3D(this.Location);
					}
					else if (o is Item)
					{
						Item item = (Item)o;
		...
		public bool CheckRegionAssignment
		{
			...
			set
			{
				if (value == true)
				{
					...
					if (this.m_Region == null && this.m_RegionName != null && this.RegionName != string.Empty)
					{
						this.RegionName = this.RegionName;

						if (this.SpawnRegion != null)
						{
		...
		public void Reset()
		{
			...
			this.m_killcount = 0;
			this.OnHold = false;
			this.mostRecentSpawnPosition = Point3D.Zero;
			this.spawnPositionWayTable = null;
		...
		public bool Respawn()
		{
			...
			bool keepProximityActivated = this.m_proximityActivated;
			...
			for (int x = 0; x < this.m_Count; x++)
			{
				...
				if (x < this.m_Count - 1 || this.OnHold)
					this.m_proximityActivated = keepProximityActivated;
			}
			if (!this.FreeRun)
			{
				this.m_mob_who_triggered = null;
		...
		public void SortSpawns()
		{
			...
			this.m_SpawnObjects.Sort(new SubgroupSorter());
		}
		...
		public bool HasSubGroups()
		{
			...
			for (int j = 0; j < this.m_SpawnObjects.Count; j++)
			{
				if (((SpawnObject)this.m_SpawnObjects[j]).SubGroup > 0)
					return true;
			}
		...
		private void RefreshNextSpawnTimes()
		{
			if (this.m_SpawnObjects != null && this.m_SpawnObjects.Count > 0)
			{
				for (int i = 0; i < this.m_SpawnObjects.Count; i++)
				{
					SpawnObject so = this.m_SpawnObjects[i] as SpawnObject;
		...
		public bool HasHoldSmartSpawning
		{
			get
			{
				...
				foreach (SpawnObject so in this.m_SpawnObjects)
				{
					...
					{
						object o = so.SpawnedObjects[x];
						if (this.CheckHoldSmartSpawning(o))
						{
							return true;
						}
					}
		...
		public bool CanSpawnMobile(int x, int y, int z, Mobile mob)
		{
			...
			return this.CanFit(x, y, z, 16, false, true, true, mob);
		}
		...
		private void FindRegionTileLocations(ref List<Point3D> locations, Region r, List<int> includetilelist, List<int> excludetilelist, TileFlag tileflag)
		{
			...
			for (int n = 0; n < count; n++)
			{
				...
				this.FindTileLocations(ref locations, r.Map, sx, sy, w, h, includetilelist, excludetilelist, tileflag);
			}
		}
		...
		{
			// no pack spawning
			return this.GetSpawnPosition(requiresurface, -1, Point3D.Zero, null, null);
		}
		...
		{
			// no pack spawning
			return this.GetSpawnPosition(requiresurface, -1, Point3D.Zero, null, mob);
		}
		...
		{
			return this.GetSpawnPosition(requiresurface, packrange, packcoord, spawnpositioning, null);
		}
		...
		public Point3D GetSpawnPosition(bool requiresurface, int packrange, Point3D packcoord, List<SpawnPositionInfo> spawnpositioning, Mobile mob)
		{
					...
					switch (s.positionType)
					{
						...
						case SpawnPositionType.Waypoint:
							...
							if (prefix != null)
							{
								...
								if (this.spawnPositionWayTable == null)
								{
									this.spawnPositionWayTable = new Hashtable();
								}

								// try to find the waypoint list in the local table
								WayList = (ArrayList)this.spawnPositionWayTable[prefix];
								...
								if (WayList == null)
								{
									...
									foreach (Item i in World.Items.Values)
									{
										if (i is WayPoint && i.Name != null && i.Map == this.Map && i.Name.StartsWith(prefix))
										{
									...
									this.spawnPositionWayTable.Add(prefix, WayList);
								}
							}
							break;
			...
			{
				if (this.m_Region != null && this.HasRegionPoints(this.m_Region))
				{
					this.FindRegionTileLocations(ref locations, this.m_Region, includetilelist, excludetilelist, tileflag);
				}
				else if (positioning == SpawnPositionType.Random)
				{
					this.FindTileLocations(ref locations, this.Map, this.m_X, this.m_Y, this.m_Width, this.m_Height, includetilelist, excludetilelist, tileflag);
				}
			}
			...
			for (int i = 0; i < 10; i++)
			{
				...
				else if (this.m_Region != null && this.HasRegionPoints(this.m_Region))	// 2004.02.08 :: Omega Red
				{
					...
					{
						Point2D p = this.GetRandomRegionPoint(this.m_Region);
						x = p.X;
						y = p.Y;
					}
				}
				else
				{
					switch (positioning)
					{
						case SpawnPositionType.Random:
							...
							{
								if (this.m_Width > 0)
									x = this.m_X + Utility.Random(this.m_Width + 1);
								if (this.m_Height > 0)
									y = this.m_Y + Utility.Random(this.m_Height + 1);
							}
							break;
						case SpawnPositionType.RelXY:

							x = this.mostRecentSpawnPosition.X + xinc;
							y = this.mostRecentSpawnPosition.Y + yinc;
							defaultZ = this.mostRecentSpawnPosition.Z + zinc;
							break;
						case SpawnPositionType.DeltaLocation:
						...
						case SpawnPositionType.RowFill:
							...
							x = this.mostRecentSpawnPosition.X + fillinc;
							y = this.mostRecentSpawnPosition.Y;
							...
							if (x > this.m_X + this.m_Width)
							{
								x = this.m_X + (x - this.m_X - this.m_Width - 1);
								y++;
							}

							if (y > this.m_Y + this.m_Height)
							{
								y = this.m_Y;
							}
						...
						case SpawnPositionType.ColFill:
							...
							x = this.mostRecentSpawnPosition.X;
							y = this.mostRecentSpawnPosition.Y + fillinc;
							...
							if (y > this.m_Y + this.m_Height)
							{
								y = this.m_Y + (y - this.m_Y - this.m_Height - 1);
								x++;
							}
						...
						case SpawnPositionType.Perimeter:
							...
							if (y == this.m_Y && x < this.m_X + this.m_Width)
								x += fillinc;
							else if (y == this.m_Y + this.m_Height && x > this.m_X)
								x -= fillinc;
							else if (x == this.m_X && y > this.m_Y)
								y -= fillinc;
							else if (x == this.m_X + this.m_Width && y < this.m_Y + this.m_Height)
								y += fillinc;
					...
					this.mostRecentSpawnPosition = new Point3D(x, y, defaultZ);
				}
				...
				if (requiresurface)
				{
					fit = this.CanSpawnMobile(x, y, defaultZ, mob);
				}
				else
				{
					fit = this.Map.CanFit(x, y, defaultZ, SpawnFitSize, true, false, false);
				}
				...
				else
				{
					...
					if (requiresurface)
					{
						fit = this.CanSpawnMobile(x, y, z, mob);
					}
					else
					{
						fit = this.Map.CanFit(x, y, z, SpawnFitSize, true, false, false);
					}
		...
		public int GetCreatureMax(int index)
		{
			...
			return ((SpawnObject)this.m_SpawnObjects[index]).MaxCount;
		}
		...
		public void AddSpawnObject(string SpawnObjectName)
		{
			...
			foreach (SpawnObject so in this.m_SpawnObjects)
			{
				if (so.TypeName.ToUpper() == SpawnObjectName.ToUpper())
				{
					...
					if (this.Running)
						this.Spawn(SpawnObjectName, false);
				}
			}
			...
			try
			{
				...
				if (TheSpawn != null)
				{
					...
					if (delete_this_entry)
					{
						...
						if (from != null)
							CommandLogging.WriteLine(from, "{0} {1} removed from XmlSpawner {2} '{3}' [{4}, {5}] ({6}) : {7}", from.AccessLevel, CommandLogging.Format(from), this.Serial, this.Name, this.GetWorldLocation().X, this.GetWorldLocation().Y, this.Map, SpawnObjectName);
					}
				}
		...
		{
			if (this.m_SectorTimer != null)
				this.m_SectorTimer.Stop();

			this.m_SectorTimer = new SectorTimer(this, delay);

			this.m_SectorTimer.Start();
		}
		...
		private class SectorTimer : Timer
		{
			...
			{
				// check the sectors
				if (this.m_Spawner != null && !this.m_Spawner.Deleted && this.m_Spawner.Running && this.m_Spawner.IsInactivated)
				{
					if (this.m_Spawner.SmartSpawning)
					{
						if (this.m_Spawner.HasActiveSectors)
						{
							this.Stop();

							this.m_Spawner.SmartRespawn();
						}
					}
					else
					{
						...
						this.m_Spawner.IsInactivated = false;
					}
				}
				else
				{
		...
		private class WarnTimer2 : Timer
		{
			...
			{
				this.m_List.Add(new WarnEntry2(p, map, name));
			}
			...
			protected override void OnTick()
			{
				try
				{
					...
					using (StreamWriter op = new StreamWriter("badspawn.log", true))
					{
						...
						foreach (WarnEntry2 e in this.m_List)
		...
		public void DoTimer(TimeSpan delay)
		{
			...
			this.m_End = DateTime.Now + delay;

			if (this.m_Timer != null)
				this.m_Timer.Stop();

			this.m_Timer = new SpawnerTimer(this, delay);
			this.m_Timer.Start();
		}

		public void DoTimer2(TimeSpan delay)
		{
			this.m_DurEnd = DateTime.Now + delay;
			if (this.m_Duration > TimeSpan.FromMinutes(0) || (this.m_durActivated == true))
			{
				if (this.m_DurTimer != null)
					this.m_DurTimer.Stop();
				this.m_DurTimer = new InternalTimer(this, delay);
				this.m_DurTimer.Start();
				this.m_durActivated = true;
			}
		}
		...
		public void DoTimer3(TimeSpan delay)
		{
			...
			//                return;
			this.m_RefractEnd = DateTime.Now + delay;
			this.m_refractActivated = true;

			if (this.m_RefractoryTimer != null)
				this.m_RefractoryTimer.Stop();

			this.m_RefractoryTimer = new InternalTimer3(this, delay);
			this.m_RefractoryTimer.Start();
		}
		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
				if (this.m_spawner != null && !this.m_spawner.Deleted)
				{
					this.m_spawner.RemoveSpawnObjects();
					this.m_spawner.m_durActivated = false;
				}
			}
		}
		...
		private class SpawnerTimer : Timer
		{
				...
				: base(delay)
			{
				...
				{
					this.Priority = spawner.BasePriority;
				}
		...
		private class InternalTimer3 : Timer
		{
			...
			protected override void OnTick()
			{
				...
				{
					// reenable triggering
					this.m_spawner.m_refractActivated = false;
				}
			}
		}
		#endregion
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			// Version 30
			writer.Write(this.m_AllowNPCTriggering);

			// Version 29
			if (this.m_SpawnObjects != null)
			{
				writer.Write(this.m_SpawnObjects.Count);
				for (int i = 0; i < this.m_SpawnObjects.Count; ++i)
				{
					// Write the spawns per tick value
					writer.Write(((SpawnObject)this.m_SpawnObjects[i]).SpawnsPerTick);
				}
			}
			else
			{
			...
			if (this.m_SpawnObjects != null)
			{
				for (int i = 0; i < this.m_SpawnObjects.Count; ++i)
				{
					...
					writer.Write(((SpawnObject)this.m_SpawnObjects[i]).PackRange);
				}
			}
			...
			if (this.m_SpawnObjects != null)
			{
				for (int i = 0; i < this.m_SpawnObjects.Count; ++i)
				{
					...
					writer.Write(((SpawnObject)this.m_SpawnObjects[i]).Disabled);
				}
			}

			// Version 26
			writer.Write(this.m_SpawnOnTrigger);
			writer.Write(this.m_FirstModified);
			...
			if (this.m_SpawnObjects != null)
			{
				for (int i = 0; i < this.m_SpawnObjects.Count; ++i)
				{
					SpawnObject so = (SpawnObject)this.m_SpawnObjects[i];
			...
			if (this.m_ShowBoundsItems != null && this.m_ShowBoundsItems.Count > 0)
			{
				writer.Write(true);
				writer.WriteItemList(this.m_ShowBoundsItems);
			}
			else
			{
			...
			// Version 23
			writer.Write(this.IsInactivated);
			writer.Write(this.m_SmartSpawning);
			// Version 22
			writer.Write(this.m_SkillTrigger);
			writer.Write((int)this.m_skill_that_triggered);
			writer.Write(this.m_FreeRun);
			writer.Write(this.m_mob_who_triggered);
			// Version 21
			writer.Write(this.m_DespawnTime);
			// Version 20
			if (this.m_SpawnObjects != null)
			{
				for (int i = 0; i < this.m_SpawnObjects.Count; ++i)
				{
					...
					writer.Write(((SpawnObject)this.m_SpawnObjects[i]).RequireSurface);
				}
			}
			// Version 19
			writer.Write(this.m_ConfigFile);
			writer.Write(this.m_OnHold);
			writer.Write(this.m_HoldSequence);
			writer.Write(this.m_FirstModifiedBy);
			writer.Write(this.m_LastModifiedBy);
			// compute the number of tags to save
			int tagcount = 0;
			for (int i = 0; i < this.m_KeywordTagList.Count; i++)
			{
				// only save WAIT type keywords or other keywords that have the save flag set
				if ((((BaseXmlSpawner.KeywordTag)(this.m_KeywordTagList[i])).Flags & BaseXmlSpawner.KeywordFlags.Serialize) != 0)
					tagcount++;
			}
			...
			for (int i = 0; i < this.m_KeywordTagList.Count; i++)
			{
				if ((((BaseXmlSpawner.KeywordTag)(this.m_KeywordTagList[i])).Flags & BaseXmlSpawner.KeywordFlags.Serialize) != 0)
				{
					((BaseXmlSpawner.KeywordTag)(this.m_KeywordTagList[i])).Serialize(writer);
				}
			}
			// Version 18
			...
			// Version 16
			writer.Write(this.m_SequentialSpawning);
			// write out the remaining time until sequential reset
			writer.Write(this.NextSeqReset);
			...
			writer.Write(this.m_RegionName);	// 2004.02.08 :: Omega Red

			// Version 15
			writer.Write(this.m_ExternalTriggering);
			writer.Write(this.m_ExternalTrigger);

			// Version 14
			writer.Write(this.m_NoItemTriggerName);

			// Version 13
			writer.Write(this.m_GumpState);

			// Version 12
			int todtype = (int)this.m_TODMode;
			writer.Write(todtype);

			// Version 11
			writer.Write(this.m_KillReset);
			writer.Write(this.m_skipped);
			writer.Write(this.m_spawncheck);

			// Version 10
			writer.Write(this.m_SetPropertyItem);

			// Version 9
			writer.Write(this.m_TriggerProbability);

			// Version 8
			writer.Write(this.m_MobPropertyName);
			writer.Write(this.m_MobTriggerName);
			writer.Write(this.m_PlayerPropertyName);

			// Version 7
			writer.Write(this.m_SpeechTrigger);

			// Version 6
			writer.Write(this.m_ItemTriggerName);

			// Version 5
			writer.Write(this.m_ProximityTriggerMessage);
			writer.Write(this.m_ObjectPropertyItem);
			writer.Write(this.m_ObjectPropertyName);
			writer.Write(this.m_killcount);

			// Version 4
			writer.Write(this.m_ProximityRange);
			writer.Write(this.m_ProximityTriggerSound);
			writer.Write(this.m_proximityActivated);
			writer.Write(this.m_durActivated);
			writer.Write(this.m_refractActivated);
			writer.Write(this.m_StackAmount);
			writer.Write(this.m_TODStart);
			writer.Write(this.m_TODEnd);
			writer.Write(this.m_MinRefractory);
			writer.Write(this.m_MaxRefractory);
			if (this.m_refractActivated)
				writer.Write(this.m_RefractEnd - DateTime.Now);
			if (this.m_durActivated)
				writer.Write(this.m_DurEnd - DateTime.Now);
			// Version 3
			writer.Write(this.m_ShowContainerStatic);
			// Version 2
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				case 30:
					{
						this.m_AllowNPCTriggering = reader.ReadBool();
						goto case 29;
					}
				case 29:
					{
				...
				case 26:
					{
						this.m_SpawnOnTrigger = reader.ReadBool();
						this.m_FirstModified = reader.ReadDateTime();
						this.m_LastModified = reader.ReadDateTime();
						goto case 25;
					}
				case 25:
					{
				...
				case 24:
					{
						...
						if (hasitems)
						{
							this.m_ShowBoundsItems = reader.ReadItemList();
						}
						goto case 23;
					}
				case 23:
					{
						this.IsInactivated = reader.ReadBool();
						this.SmartSpawning = reader.ReadBool();

						goto case 22;
					}
				case 22:
					{
						this.SkillTrigger = reader.ReadString();    // note this will also register the skill
						this.m_skill_that_triggered = (SkillName)reader.ReadInt();
						this.m_FreeRun = reader.ReadBool();
						this.m_mob_who_triggered = reader.ReadMobile();
						goto case 21;
					}
				case 21:
					{
						this.m_DespawnTime = reader.ReadTimeSpan();
						goto case 20;
					}
				case 20:
					{
				...
				case 19:
					{
						...
						this.m_OnHold = reader.ReadBool();
						this.m_HoldSequence = reader.ReadBool();
						this.m_FirstModifiedBy = reader.ReadString();
						this.m_LastModifiedBy = reader.ReadString();
						// deserialize the keyword tag list
						int tagcount = reader.ReadInt();
						this.m_KeywordTagList = new ArrayList(tagcount);
				...
				case 16:
					{
						...
						this.m_SequentialSpawning = reader.ReadInt();
						TimeSpan seqdelay = reader.ReadTimeSpan();
						this.m_SeqEnd = DateTime.Now + seqdelay;
						if (version < 20)
						{
							tmpSpawnListSize = reader.ReadInt();
						}
						...
						this.m_RegionName = reader.ReadString();	// 2004.02.08 :: Omega Red
						goto case 15;
					}
				case 15:
					{
						this.m_ExternalTriggering = reader.ReadBool();
						this.m_ExternalTrigger = reader.ReadBool();
						goto case 14;
					}
				case 14:
					{
						this.m_NoItemTriggerName = reader.ReadString();
						goto case 13;
					}
				case 13:
					{
						this.m_GumpState = reader.ReadString();
						goto case 12;
					}
				case 12:
					{
						...
						{
							case (int)TODModeType.Gametime:
								this.m_TODMode = TODModeType.Gametime;
								break;
							case (int)TODModeType.Realtime:
								this.m_TODMode = TODModeType.Realtime;
								break;
						}
						goto case 11;
					}
				case 11:
					{
						this.m_KillReset = reader.ReadInt();
						this.m_skipped = reader.ReadBool();
						this.m_spawncheck = reader.ReadInt();
						goto case 10;
					}
				case 10:
					{
						this.m_SetPropertyItem = reader.ReadItem();
						goto case 9;
					}
				case 9:
					{
				...
				case 8:
					{
						...
						this.m_MobTriggerName = reader.ReadString();
						this.m_PlayerPropertyName = reader.ReadString();
						goto case 7;
					}
				case 7:
					{
						this.m_SpeechTrigger = reader.ReadString();
						goto case 6;
					}
				case 6:
					{
						this.m_ItemTriggerName = reader.ReadString();
						goto case 5;
					}
				case 5:
					{
						this.m_ProximityTriggerMessage = reader.ReadString();
						this.m_ObjectPropertyItem = reader.ReadItem();
						this.m_ObjectPropertyName = reader.ReadString();
						this.m_killcount = reader.ReadInt();
						goto case 4;
					}
				case 4:
					{
						...
						this.m_ProximityTriggerSound = reader.ReadInt();
						this.m_proximityActivated = reader.ReadBool();
						this.m_durActivated = reader.ReadBool();
						this.m_refractActivated = reader.ReadBool();
						this.m_StackAmount = reader.ReadInt();
						this.m_TODStart = reader.ReadTimeSpan();
						this.m_TODEnd = reader.ReadTimeSpan();
						this.m_MinRefractory = reader.ReadTimeSpan();
						this.m_MaxRefractory = reader.ReadTimeSpan();
						if (this.m_refractActivated == true)
						{
							TimeSpan delay = reader.ReadTimeSpan();
							this.DoTimer3(delay);
						}
						if (this.m_durActivated == true)
						{
							TimeSpan delay = reader.ReadTimeSpan();
							this.DoTimer2(delay);
						}
				...
				case 3:
					{
						this.m_ShowContainerStatic = reader.ReadItem() as Static;
						goto case 2;
					}
				case 2:
					{
		...
		public class SpawnObject
		{
			...
			public int SpawnsPerTick
			{
				get
				{
					return this.m_SpawnsPerTick;
				}
				set
				{
					this.m_SpawnsPerTick = value;
				}
			}
			public int SequentialResetTo
			{
				get
				{
					return this.m_SequentialResetTo;
				}
				set
				{
					this.m_SequentialResetTo = value;
				}
			}
			public int KillsNeeded
			{
				get
				{
					return this.m_KillsNeeded;
				}
				set
				{
					this.m_KillsNeeded = value;
				}
			}
			public bool RestrictKillsToSubgroup
			{
				get
				{
					return this.m_RestrictKillsToSubgroup;
				}
				set
				{
					this.m_RestrictKillsToSubgroup = value;
				}
			}
			public bool ClearOnAdvance
			{
				get
				{
					return this.m_ClearOnAdvance;
				}
				set
				{
					this.m_ClearOnAdvance = value;
				}
			}
			...
			public int PackRange
			{
				get
				{
					return this.m_PackRange;
				}
				set
				{
					this.m_PackRange = value;
				}
			}
