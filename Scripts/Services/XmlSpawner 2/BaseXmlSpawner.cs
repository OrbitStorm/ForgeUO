...
namespace Server.Mobiles
{
    ...
    public class BaseXmlSpawner
    {
        ...
        private static readonly ArrayList ProtectedPropertiesList = new ArrayList();
        ...
        {
            if (typeName == null || typeName.Length < 1 || !Char.IsUpper(typeName[0]))
                return false;

            if (itemKeywordHash == null)
                InitializeHash();
        ...
        public static bool IsTypeOrItemKeyword(string typeName)
        {
            ...
            if (typeKeywordHash == null || itemKeywordHash == null)
                InitializeHash();
        ...
        public class KeywordTag
        {
            ...
            public KeywordTag(string typename, XmlSpawner spawner, int type, TimeSpan delay, TimeSpan timeout, string condition, int gotogroup)
            {
                ...
                this.m_TimeoutEnd = DateTime.Now + timeout;
                this.m_Spawner = spawner;
                this.m_Condition = condition;
                this.m_Goto = gotogroup;

                this.Typename = typename;
                // add the tag to the list
                if (spawner != null && !spawner.Deleted)
                {
                    this.m_TrigMob = spawner.TriggerMob;
                    ...
                    if (spawner.m_KeywordTagList.Count > 0)
                        this.Serial = ((KeywordTag)(spawner.m_KeywordTagList[spawner.m_KeywordTagList.Count - 1])).Serial + 1;
                    else
                        this.Serial = 0;
                    ...
                    switch (type)
                    {
                        case 0: // WAIT timer type
                            // start up the timer
                            this.DoTimer(delay, this.m_Delay, condition, gotogroup);
                            // and put spawning on hold until it is done
                            //spawner.OnHold = true;
                            this.Flags |= KeywordFlags.HoldSpawn;
                            this.Flags |= KeywordFlags.Serialize;
                        ...
                        case 1: // GUMP type
                            ...
                            break;
                        case 2: // GOTO type
                            this.Flags |= KeywordFlags.HoldSequence;
                            this.Flags |= KeywordFlags.Serialize;

                            break;
                        default:
                            // dont do anything for other types
                            this.Flags |= KeywordFlags.Defrag;
                            break;
                    }
                }
            }
            ...
            public void Delete()
            {
                ...
                //}
                // and stop all timers
                if (this.m_Timer != null && this.Type == 0)
                {
                    this.m_Timer.Stop();
                }
                ...
                RemoveFromTagList(this.m_Spawner, this);
            }
            ...
            private void DoTimer(TimeSpan delay, TimeSpan repeatdelay, string condition, int gotogroup)
            {
                ...
                this.m_Timer = new KeywordTimer(this.m_Spawner, this, delay, repeatdelay, condition, gotogroup);
                this.m_Timer.Start();
            }
            ...
            public void Serialize(GenericWriter writer)
            {
                ...
                if (this.Type == 0)
                {
                    ...
                    writer.Write(this.m_Goto);
                    writer.Write(this.m_TimeoutEnd - DateTime.Now);
                    writer.Write(this.m_Timeout);
                    writer.Write(this.m_TrigMob);
                }
            }
            ...
            private class KeywordTimer : Timer
            {
                    ...
                    : base(delay)
                {
                    ...
                    this.m_Condition = condition;
                    this.m_Goto = gotogroup;
                    this.m_Repeatdelay = repeatdelay;
                }

                protected override void OnTick()
                {
                    // if a condition is available then test it
                    if (this.m_Condition != null && this.m_Condition.Length > 0 && this.m_Spawner != null && this.m_Spawner.Running)
                    {
                        ...
                        {
                            trigmob = this.m_Spawner.TriggerMob;
                        }

                        if (TestItemProperty(this.m_Spawner, this.m_Spawner, this.m_Condition, trigmob, out status_str))
                        {
                            // release the hold on spawning
                            //m_Spawner.OnHold = false;
                            // spawn the designated subgroup if specified
                            if (this.m_Goto >= 0 && this.m_Spawner != null && !this.m_Spawner.Deleted)
                            {
                                // set the trigmob to the mob that originally triggered the wait keyword
                                if (this.m_Tag != null)
                                    this.m_Spawner.TriggerMob = this.m_Tag.m_TrigMob;

                                // spawn the subgroup
                                this.m_Spawner.SpawnSubGroup(this.m_Goto);
                                // advance sequential spawning to that group
                                //m_Spawner.SequentialSpawn = m_Goto;
                            }

                            // get rid of the temporary tag
                            if (this.m_Tag != null && !this.m_Tag.Deleted)
                            {
                                this.m_Tag.Delete();
                            }
                        }
                        else
                        {
                            ...
                            if (this.m_Tag != null && !this.m_Tag.Deleted)
                            {
                                ...
                                if (this.m_Tag.m_Timeout > TimeSpan.Zero && this.m_Tag.m_TimeoutEnd < DateTime.Now)
                                {
                                ...
                                {
                                    this.m_Tag.DoTimer(this.m_Repeatdelay, this.m_Repeatdelay, this.m_Condition, this.m_Goto);
                                }
                            }
                        }
                    }
                        ...
                        else if (IsTypemodKeyword(keywordargs[0]))
                        {
                            ...
                            else if (kw == typemodKeyword.SPEECH)
                            {
                                ...
                                if (keywordargs.Length > 2)
                                {
                                    try
                                    {
                                        keyword_number = int.Parse(keywordargs[2]);
                                    }
                                    catch
                                    {
                            ...
                            else if (kw == typemodKeyword.ANIMATE)
                            {
                                ...
                                if (keywordargs.Length > 1)
                                {
                                    try
                                    {
                                        action = int.Parse(keywordargs[1]);
                                    }
                                    catch
                                    {
                                ...
                                if (keywordargs.Length > 2)
                                {
                                    try
                                    {
                                        framecount = int.Parse(keywordargs[2]);
                                    }
                                    catch
                                    {
                                ...
                                if (keywordargs.Length > 3)
                                {
                                    try
                                    {
                                        repeatcount = int.Parse(keywordargs[3]);
                                    }
                                    catch
                                    {
                                ...
                                if (keywordargs.Length > 4)
                                {
                                    try
                                    {
                                        forward = bool.Parse(keywordargs[4]);
                                    }
                                    catch
                                    {
                                ...
                                if (keywordargs.Length > 5)
                                {
                                    try
                                    {
                                        repeat = bool.Parse(keywordargs[5]);
                                    }
                                    catch
                                    {
        ...
        public static PropertyInfo LookupPropertyInfo(XmlSpawner spawner, Type type, string propname)
        {
            ...
            if (spawner.PropertyInfoList == null)
                spawner.PropertyInfoList = new ArrayList();
        ...
        public static bool CheckSingleProperty(XmlSpawner spawner, object o, string testString, Mobile trigmob, out string status_str)
        {
            ...
            if (ptype2 == typeof(TimeSpan) || ptype1 == typeof(TimeSpan))
            {
                if (hasequal)
                {
                    try
                    {
                        if (TimeSpan.Parse(value1) == TimeSpan.Parse(value2))
                            return !invertreturn;
                    }
                    catch
                    {
            ...
            else if (ptype2 == typeof(Boolean) && ptype1 == typeof(Boolean))
            {
                if (hasequal)
                {
                    try
                    {
                        if (Convert.ToBoolean(value1) == Convert.ToBoolean(value2))
                            return !invertreturn;
                    }
                    catch (Exception se)
                    {
                        status_str = "invalid bool comparison : {0}" + testString + se.Message;
                    }
                }
                else if (hasnotequals)
                {
        ...
        public sealed class XmlPlayMusic : Packet
        {
            public XmlPlayMusic(short number)
                : base(0x6D, 3)
            {
                this.UnderlyingStream.Write(number);
            }
        }
