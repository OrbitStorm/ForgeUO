...
namespace Server.Engines.XmlSpawner2
{
    public class XmlStamDrain : XmlAttachment
    {
        ...
        public XmlStamDrain(ASerial serial)
            : base(serial)
        {
        }
        ...
        public override string OnIdentify(Mobile from)
        {
            ...
            {
                msg = String.Format("Stamina drain {0} expires in {1} mins", this.m_Drain, this.Expiration.TotalMinutes);
            }
            else
            {
                msg = String.Format("Stamina drain {0}", this.m_Drain);
            }
        ...
        public override void OnAttach()
        {
            ...
            {
                if (this.m_Drain > 0)
                    ((Mobile)this.AttachedTo).SendMessage("You have been granted the power of Stamina Drain!");
                else
                    ((Mobile)this.AttachedTo).SendMessage("You have been cursed with Stamina Drain!");
            }
        }
