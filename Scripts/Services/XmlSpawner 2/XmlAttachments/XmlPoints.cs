...
namespace Server.Engines.XmlSpawner2
{
    public class XmlPoints : XmlAttachment
    {
        ...
        public bool ReceiveBroadcasts
        {
            get
            {
                return this.m_ReceiveBroadcasts;
            }
            set
            {
                this.m_ReceiveBroadcasts = value;
            }
        }
        ...
        public bool HasChallenge
        {
            get
            {
                return ((this.m_Challenger != null && !this.m_Challenger.Deleted) || (this.m_ChallengeGame != null && !this.m_ChallengeGame.Deleted));
            }
        }
        ...
        public BaseChallengeGame ChallengeGame
        {
            get
            {
                return this.m_ChallengeGame;
            }
            set
            {
                this.m_ChallengeGame = value;
            }
        }

        public BaseChallengeGame ChallengeSetup
        {
            get
            {
                return this.m_ChallengeSetup;
            }
            set
            {
                this.m_ChallengeSetup = value;
            }
        }

        public static bool TeleportOnDuel = true;                          // are players automatically teleported to and from the specified dueling location

        private static readonly TimeSpan m_DeathDelay = TimeSpan.FromSeconds(60);    // 60 seconds default min time between deaths for point loss
        private static readonly TimeSpan m_KillDelay = TimeSpan.FromHours(6);       // 6 hour default min interval between kills of the same player for point gain
        ...
        private static readonly double m_WinScale = 0.05;                               // set to zero for no scaling for points gained for killing (fixed 1 point per kill)
        private static readonly double m_LoseScale = 0.05;                              // set to zero for no scaling for points lost when killed (fixed 1 point per death)
        private static readonly double m_CreditScale = 0.05;
        ...
        private static readonly string m_LeaderboardSaveDirectory = "Leaderboard";     // default directory for saving leaderboard xml information
        ...
        private static readonly TimeSpan m_PointsDecayTime = TimeSpan.FromDays(15);      // default time interval for automatic point loss for no pvp activity
        // set m_PointsDecay to zero to disable the automatic points decay feature
        private static readonly int m_PointsDecay = 10;                                  // default point loss if no kills are made within the PointsDecayTime
        ...
        private static readonly LanguageType m_SystemLanguage = LanguageType.ENGLISH; // system default language setting
        ...
        public class DuelLocationEntry
        {
            ...
            {
                this.Name = name;
                this.DuelLocation = new Point3D(X,Y,Z);
                this.DuelMap = map;
                this.DuelRange = range;
            }
        }
        ...
        {
            if ((int)this.m_CurrentLanguage >= TextHash.Length)
            {
                // unsupported language
                // this should never happen
                Console.WriteLine("XmlPoints: Unsupported language {0}", this.m_CurrentLanguage);
                return String.Format("??Language {0}??", this.m_CurrentLanguage);
            }

            Hashtable h = TextHash[(int)this.m_CurrentLanguage];

            if (h == null || !h.Contains(index))
            {
                Console.WriteLine("XmlPoints: Missing entry for {0} in language {1}", index, this.m_CurrentLanguage);
        ...
        public void SendColorText(int color, int msgindex)
        {
            ...
            {
                from = (Mobile)this.AttachedTo;
            }
            // go through the participant list and send all participants the message
            if (from == null)
                return;

            from.SendMessage(color, String.Format(this.Text(msgindex)));
        }
        ...
        public void SendColorText(int color, int msgindex, object arg, object arg2)
        {
            ...
            from.SendMessage(color, String.Format(this.Text(msgindex), arg, arg2));
        }
        ...
        public void SendColorText(int color, int msgindex, object arg)
        {
            ...
            from.SendMessage(color, String.Format(this.Text(msgindex), arg));
        }
        ...
        {
            public readonly Mobile Killed;
            public readonly DateTime WhenKilled;
        ...
        public class RankEntry : IComparable
        {
            ...
            {
                this.Killer = m;
                this.PointsAttachment = attachment;
            }
            ...
            public int CompareTo(object obj)
            {
                ...
                if (p.PointsAttachment == null || this.PointsAttachment == null)
                    return 0;

                // break points ties with kills (more kills means higher rank)
                if (p.PointsAttachment.Points - this.PointsAttachment.Points == 0)
                {
                    // if kills are the same then compare deaths (fewer deaths means higher rank)
                    if (p.PointsAttachment.Kills - this.PointsAttachment.Kills == 0)
                    {
                        // if deaths are the same then use previous ranks
                        if (p.PointsAttachment.Deaths - this.PointsAttachment.Deaths == 0)
                        {
                            return p.PointsAttachment.Rank - this.PointsAttachment.Rank;
                        }

                        return this.PointsAttachment.Deaths - p.PointsAttachment.Deaths;
                    }

                    return p.PointsAttachment.Kills - this.PointsAttachment.Kills;
                }

                return p.PointsAttachment.Points - this.PointsAttachment.Points;
            }
        }
        ...
        private class LeaderboardTimer : Timer
        {
                ...
                : base(delay, delay)
            {
                ...
                this.m_nranks = nranks;
                this.Priority = TimerPriority.FiveSeconds;
            }

            protected override void OnTick()
            {
                WriteLeaderboard(this.m_filename, this.m_nranks);
            }
        }
        ...
        public void DoTimer(TimeSpan delay)
        {
            ...
            this.m_CancelEnd = DateTime.Now + delay;
            this.m_CancelTimer.Start();
        }

        public class CancelTimer : Timer
        {
            private readonly XmlPoints m_attachment;

            public CancelTimer(XmlPoints a, TimeSpan delay)
                : base(delay)
            {
            ...
            protected override void OnTick()
            {
                ...
                Mobile from = this.m_attachment.AttachedTo as Mobile;
			     
                if (from != null && this.m_attachment.Challenger != null)
                {
                    SendText(from, 100214, this.m_attachment.Challenger.Name); // "Challenge with {0} has been cancelled"
			         
                    if (from.HasGump(typeof(PointsGump)))
                    {
                        this.m_attachment.OnIdentify(from);
                    }
                }

                // clear the challenger on the challengers attachment
                XmlPoints xa = (XmlPoints)XmlAttach.FindAttachment(this.m_attachment.Challenger, typeof(XmlPoints));
                ...
                if (xa != null && !xa.Deleted)
                {
                    ...
                    {
                        if (this.m_attachment.Challenger != null && from != null)
                        {
                            this.m_attachment.Challenger.SendMessage(String.Format(xa.Text(100214), from.Name)); // "Challenge with {0} has been cancelled"
                        }
                        // then clear it
                        xa.Challenger = null;
                    }
                ...
                this.m_attachment.Challenger = null;
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (this.m_Kills > 0 && m_PointsDecay > 0 && this.m_Points > DefaultStartingPoints && (DateTime.Now - this.m_LastDecay) > m_PointsDecayTime &&
                (DateTime.Now - this.m_LastKill) > m_PointsDecayTime && (DateTime.Now - this.m_LastDeath) > m_PointsDecayTime)
            {
                this.m_Points -= m_PointsDecay;
                if (this.m_Points < DefaultStartingPoints)
                    this.m_Points = DefaultStartingPoints;
                this.m_LastDecay = DateTime.Now;
            }

            writer.Write((int)8);
            // version 8
            writer.Write(this.m_StartingLoc);

            if (this.m_StartingMap != null)
            {
                writer.Write(this.m_StartingMap.ToString());
            }
            else
            {
            ...
            // version 7
            writer.Write(this.m_CurrentLanguage.ToString());
            // version 6
            writer.Write(this.m_ChallengeGame);
            writer.Write(this.m_ChallengeSetup);
            // version 5
            writer.Write(this.m_CancelEnd - DateTime.Now);
            // version 4
            writer.Write(this.m_ReceiveBroadcasts);
            // version 3
            writer.Write(this.m_Rank);
            writer.Write(this.m_DeltaRank);
            writer.Write(this.m_WhenRanked);
            writer.Write(this.m_LastDecay);
            // version 2
            writer.Write(this.m_Credits);
            // version 1
            writer.Write(this.m_Broadcast);
            // version 0
            writer.Write(this.m_Points);
            writer.Write(this.m_Kills);
            writer.Write(this.m_Deaths);
            writer.Write(this.m_Challenger);
            writer.Write(this.m_LastKill);
            writer.Write(this.m_LastDeath);
            // write out the kill list
            if (this.KillList != null)
            {
                writer.Write(this.KillList.Count);
                foreach (KillEntry k in this.KillList)
                {
        ...
        public bool CanAffectPoints(Mobile from, Mobile killer, Mobile killed, bool assumechallenge)
        {
            ...
            if ((from == killer && this.m_Challenger == killed) || (from == killed && this.m_Challenger == killer))
            {
                inchallenge = true;
            }
            ...
            if (this.ChallengeGame != null && !this.ChallengeGame.Deleted)
            {
                ...
                if (!this.ChallengeGame.AllowPoints)
                    return false;

                inchallenge = true;
                
                // check for kill delay limitations on points awards
                norestriction = !this.ChallengeGame.UseKillDelay;
            }
            ...
            if (from == killed)
            {
                ...
                if (DateTime.Now < this.m_LastDeath + m_DeathDelay)
                    return false;
            }
        ...
        public override void OnKilled(Mobile killed, Mobile killer)
        {
            ...
            if (killed == this.m_Challenger || killer == this.m_Challenger || AreInSameGame(killer, killed))
            {
                this.ClearAggression(killer, killed);
            ...
            // handle challenge team kills
            if (this.ChallengeGame != null && !this.ChallengeGame.Deleted && this.ChallengeGame.GetParticipant(killer) != null && this.ChallengeGame.GetParticipant(killed) != null)
            {
                this.ChallengeGame.OnPlayerKilled(killer, killed);
            }
            ...
            if (killed == this.m_Challenger || killer == this.m_Challenger)
            {
                ...
                if (TeleportOnDuel)
                {
                    ...
                    Timer.DelayCall(TimeSpan.FromSeconds(7), new TimerStateCallback(Return_Callback),
                        new object[] { killer, killed, this.m_StartingLoc, this.m_StartingMap });
                }
            }
            ...
            int val = (int)((this.Points - killerpoints) * m_LoseScale);
            if (val <= 0)
                val = 1;

            int startpoints = this.Points;
            ...
            if (startpoints - this.Points > 0)
            {
                ...
                Timer.DelayCall(TimeSpan.FromSeconds(5), new TimerStateCallback(ReportPointLoss_Callback),
                    new object[] { startpoints - this.Points, killer.Name, killed });

                // update the overall ranking list
                UpdateRanking(killed, this);
            }

            this.m_LastDeath = DateTime.Now;
        }
        ...
        public override void OnAttach()
        {
            ...
            if (!(this.AttachedTo is Mobile && ((Mobile)this.AttachedTo).Player))
                this.Delete();
        }
        ...
        public override string OnIdentify(Mobile from)
        {
            ...
            int val = GetRanking((Mobile)this.AttachedTo);

            StringBuilder msg = new StringBuilder();

            if (val > 0)
            {
                msg.AppendFormat(GetText(from, 100218), this.Points);  // "Current Points = {0}"
                msg.AppendFormat("\n");
            ...
            if ((from != null) && ((this.AttachedTo == from) || (from.AccessLevel > AccessLevel.Player)))
            {
                msg.AppendFormat("\n");
                msg.AppendFormat(GetText(from, 100221), this.Credits);  // "Available Credits = {0}"
            }

            msg.AppendFormat("\n");
            msg.AppendFormat(GetText(from, 100222), this.Kills, this.Deaths); // "Total Kills = {0}\nTotal Deaths = {1}\nRecent Kill List"

            this.RefreshKillList();

            if (this.KillList != null && this.KillList.Count > 0)
            {
                foreach (KillEntry k in this.KillList)
                {
            ...
            if (from != null)
            {
                ...
                from.SendGump(new PointsGump(this, from, (Mobile)this.AttachedTo, msg.ToString()));
            }
        ...
        */
        ...
        {
            readonly XmlPoints m_attachment;
            readonly Mobile m_target;
            readonly string m_text;

            public PointsGump(XmlPoints a, Mobile from, Mobile target, string text)
                : base(0,0)
            {
                ...
                if (from == target)
                {
                    this.AddBackground(0, 0, 440, 295, 5054);
                    this.AddAlphaRegion(0, 0, 440, 295);
                }
                else
                {
                    this.AddBackground(0, 0, 440, 190, 5054);
                    this.AddAlphaRegion(0, 0, 440, 190);
                }
                this.AddLabel(20, 2, 55, String.Format(GetText(from, 200224), target.Name));  // "Points Standing for {0}"
                ...
                if (a.Challenger != null)
                {
                    ...
                    this.AddLabel(20, 143, challengehue, String.Format(GetText(from, 200225), a.Challenger.Name));  // "Currently challenging {0}"
                }
                else if (a.ChallengeGame != null && !a.ChallengeGame.Deleted)
                {
                    this.AddLabel(50, 143, 68, String.Format("{0}", a.ChallengeGame.ChallengeName));
                    // add the info button that will open the game gump
                    this.AddButton(23, 143, 0x5689, 0x568A, 310, GumpButtonType.Reply, 0);
                }

                this.AddHtml(20, 20, 400, 120, text, true, true);
                ...
                if (from == target)
                {
                    ...
                    this.AddButton(x1, 165, (a.ReceiveBroadcasts ? 0xD3 : 0xD2), (a.ReceiveBroadcasts ? 0xD2 : 0xD3), 100, GumpButtonType.Reply, 0);

                    // add the broadcast kills checkbox
                    this.AddLabel(x2 + 30, 165, 55, a.Text(200227));  // "Broadcast kills"
                    this.AddButton(x2, 165, (a.Broadcast ? 0xD3 : 0xD2), (a.Broadcast ? 0xD2 : 0xD3), 200, GumpButtonType.Reply, 0);

                    // add the topplayers button
                    this.AddLabel(x3 + 30, 165, 55, a.Text(200228));  // "Top players"
                    this.AddButton(x3, 165, 0xFAB, 0xFAD, 300, GumpButtonType.Reply, 0);

                    // add the challenge button
                    this.AddLabel(x1 + 30, 190, 55, a.Text(200229));   // "Challenge"
                    this.AddButton(x1, 190, 0xFAB, 0xFAD, 400, GumpButtonType.Reply, 0);

                    // add the last man standing challenge button
                    this.AddLabel(x2 + 30, 190, 55, a.Text(200230));   // "LMS"
                    this.AddButton(x2, 190, 0xFAB, 0xFAD, 401, GumpButtonType.Reply, 0);

                    // add the deathmatch challenge button
                    this.AddLabel(x3 + 30, 190, 55, a.Text(200231));  // "Deathmatch"
                    this.AddButton(x3, 190, 0xFAB, 0xFAD, 403, GumpButtonType.Reply, 0);

                    // add the kingofthehill challenge button
                    this.AddLabel(x1 + 30, 215, 55, a.Text(200232));  // "KotH"
                    this.AddButton(x1, 215, 0xFAB, 0xFAD, 404, GumpButtonType.Reply, 0);
                    ...
                    this.AddLabel(x3 + 30, 215, 55, a.Text(200234));  // "Team LMS"
                    this.AddButton(x3, 215, 0xFAB, 0xFAD, 406, GumpButtonType.Reply, 0);

                    // add the team deathmatch challenge button
                    this.AddLabel(x1 + 30, 240, 55, a.Text(200235));   // "Team DMatch"
                    this.AddButton(x1, 240, 0xFAB, 0xFAD, 407, GumpButtonType.Reply, 0);

                    // add the team deathball challenge button
                    this.AddLabel(x2 + 30, 240, 55, a.Text(200236));  // "Team DBall"
                    this.AddButton(x2, 240, 0xFAB, 0xFAD, 408, GumpButtonType.Reply, 0);

                    // add the team KotH challenge button
                    this.AddLabel(x3 + 30, 240, 55, a.Text(200237));  // "Team KotH"
                    this.AddButton(x3, 240, 0xFAB, 0xFAD, 409, GumpButtonType.Reply, 0);
                    
                    // add the CTF challenge button
                    this.AddLabel(x1 + 30, 265, 55, a.Text(200238));  // "CTF"
                    this.AddButton(x1, 265, 0xFAB, 0xFAD, 410, GumpButtonType.Reply, 0);
                }
            }
            ...
            public override void OnResponse(NetState state, RelayInfo info)
            {
                ...
                switch(info.ButtonID)
                {
                    case 100:
                        // toggle see kills
                        this.m_attachment.ReceiveBroadcasts = !this.m_attachment.ReceiveBroadcasts;

                        state.Mobile.SendGump(new PointsGump(this.m_attachment, state.Mobile, this.m_target, this.m_text));
                        break;
                    case 200:
                        // toggle broadcast my kills
                        this.m_attachment.Broadcast = !this.m_attachment.Broadcast;
                    ...
                    case 300:
                        ...
                        state.Mobile.SendGump(new TopPlayersGump(this.m_attachment));

                        state.Mobile.SendGump(new PointsGump(this.m_attachment, state.Mobile, this.m_target, this.m_text));
                        break;
                    case 310:
                        // Challenge game info
                        if (this.m_attachment.ChallengeGame != null && !this.m_attachment.ChallengeGame.Deleted)
                            this.m_attachment.ChallengeGame.OnDoubleClick(state.Mobile);
        ...
        public class TopPlayersGump : Gump
        {
            ...
            public TopPlayersGump(XmlPoints attachment)
                : base(0,0)
            {
                if (RankList == null || attachment == null)
                    return;
                ...
                this.AddAlphaRegion(0, 0, width, height);
                this.AddImageTiled(20, 20, width - 40, height - 45, 0xBBC);
                this.AddLabel(20, 2, 55, attachment.Text(200239)); // "Top Player Rankings"

                // guild filter
                this.AddLabel(40, height - 20, 55, attachment.Text(200240));  // "Filter by Guild"
                string filter = null;
                if (this.m_attachment != null)
                    filter = this.m_attachment.guildFilter;

                this.AddImageTiled(140, height - 20, 160, 19, 0xBBC);
                this.AddTextEntry(140, height - 20, 160, 19, 0, 200, filter);

                this.AddButton(20, height - 20, 0x15E1, 0x15E5, 200, GumpButtonType.Reply, 0);

                // name filter
                this.AddLabel(340, height - 20, 55, attachment.Text(200241));  // "Filter by Name"
                string nfilter = null;
                if (this.m_attachment != null)
                    nfilter = this.m_attachment.nameFilter;

                this.AddImageTiled(440, height - 20, 160, 19, 0xBBC);
                this.AddTextEntry(440, height - 20, 160, 19, 0, 100, nfilter);

                this.AddButton(320, height - 20, 0x15E1, 0x15E5, 100, GumpButtonType.Reply, 0);

                RefreshRankList();

                int xloc = 23;
                this.AddLabel(xloc, 20, 0, attachment.Text(200242));  // "Name"
                xloc += 177;
                this.AddLabel(xloc, 20, 0, attachment.Text(200243));  // "Guild"
                #if(FACTIONS)
                xloc += 35;
                this.AddLabel(xloc, 20, 0, attachment.Text(200640));  // "Faction"
                xloc += 15;
                #endif
                xloc += 50;
                this.AddLabel(xloc, 20, 0, attachment.Text(200244));   // "Points"
                xloc += 50;
                this.AddLabel(xloc, 20, 0, attachment.Text(200245));    // "Kills"
                xloc += 50;
                this.AddLabel(xloc, 20, 0, attachment.Text(200246));   // "Deaths"
                xloc += 70;
                this.AddLabel(xloc, 20, 0, attachment.Text(200247));     // "Rank"
                xloc += 45;
                this.AddLabel(xloc, 20, 0, attachment.Text(200248));   // "Change"
                xloc += 45;
                this.AddLabel(xloc, 20, 0, attachment.Text(200249));  // "Time at Rank"
                ...
                for (int i = 0; i < RankList.Count; i++)
                {
                    ...
                    if (r.Killer != null && !r.Killer.Deleted && r.Rank > 0 && a != null && !a.Deleted)
                    {
                        ...
                        if (this.m_attachment != null && this.m_attachment.guildFilter != null && this.m_attachment.guildFilter.Length > 0)
                        {
                            // parse the comma separated list
                            string[] args = this.m_attachment.guildFilter.Split(',');
                            if (args != null)
                            {
                                bool found = false;
                        ...
                        if (this.m_attachment != null && this.m_attachment.nameFilter != null && this.m_attachment.nameFilter.Length > 0)
                        {
                            // parse the comma separated list
                            string[] args = this.m_attachment.nameFilter.Split(',');
                        ...
                        xloc = 23;
                        this.AddLabel(xloc, y, 0, r.Killer.Name);
                        xloc += 177;
                        this.AddLabel(xloc, y, 0, guildname);
                        #if(FACTIONS)
                        xloc += 35;
                        this.AddLabelCropped(xloc, y, 60, 21, 0, factionname);
                        xloc += 15;
                        #endif
                        xloc += 50;
                        this.AddLabel(xloc, y, 0, a.Points.ToString());
                        xloc += 50;
                        this.AddLabel(xloc, y, 0, kills);
                        xloc += 50;
                        this.AddLabel(xloc, y, 0, deaths);
                        xloc += 70;
                        this.AddLabel(xloc, y, 0, a.Rank.ToString());
                        ...
                        xloc += 50;
                        this.AddLabel(xloc, y, deltahue, deltalabel);
                        xloc += 40;
                        this.AddLabel(xloc, y, 0, label);
            ...
            public override void OnResponse(NetState state, RelayInfo info)
            {
                ...
                if (this.m_attachment != null)
                {
                    ...
                    if (entry != null)
                        this.m_attachment.guildFilter = entry.Text;
                        
                    entry = info.GetTextEntry(100);
                    if (entry != null)
                        this.m_attachment.nameFilter = entry.Text;
                }
        ...
        private class IssueChallengeGump : Gump
        {
                ...
                : base(0, 0)
            {
                ...
                this.Dragable = true;
                this.AddPage(0);
                this.AddBackground(10, 200, 200, height, 5054);

                this.AddLabel(20, 205, 68, String.Format(GetText(from, 200254))); // "You are challenging"
                this.AddLabel(20, 225, 68, String.Format(GetText(from, 200255), target.Name));  // "{0}. Continue?"
                ...
                this.AddLabel(55, y, texthue, String.Format(GetText(from, 200660)));  // "Cancel"
                this.AddRadio(20, y, 9721, 9724, true, 0);
                y += 30;

                this.AddLabel(55, y, texthue, String.Format(GetText(from, 200661)));  // "Duel here"
                this.AddRadio(20, y, 9721, 9724, false, 1);
                y += 30;
                ...
                if (TeleportOnDuel && SpellHelper.CheckTravel(from.Map, from.Location, TravelCheckType.RecallFrom) && SpellHelper.CheckTravel(target.Map, target.Location, TravelCheckType.RecallFrom))
                {
                    for (int i = 0; i < XmlPoints.DuelLocations.Length; i++)
                    {
                        ...
                        this.AddLabel(55, y, texthue, XmlPoints.DuelLocations[i].Name);
                        this.AddRadio(20, y, 9721, 9724, false, i + 2);
                        y += 30;
                    }
                }
                ...
                {
                    this.AddLabel(20, y, 33, String.Format(GetText(from, 200256)));  // "You will NOT gain points!"
                }
                y += 30;
                /*
                y += 25;
                ...
                */
                ...
                this.AddButton(80, y, 2130, 2129, 3, GumpButtonType.Reply, 0); // Okay button
            }
                    ...
                    default:
                        {
                            ...
                            {
                                // issue the challenge
                                this.m_Target.SendGump(new ConfirmChallengeGump(this.m_From, this.m_Target, XmlPoints.DuelLocations[radiostate - 2]));
                                this.m_From.SendMessage(GetText(this.m_From, 100257), this.m_Target.Name);  // "You have issued a challenge to {0}."
                            }
                            else if (radiostate == 1)
                            {
                                // issue the challenge
                                this.m_Target.SendGump(new ConfirmChallengeGump(this.m_From, this.m_Target, null));
                                this.m_From.SendMessage(GetText(this.m_From, 100257), this.m_Target.Name);  // "You have issued a challenge to {0}."
                            }
                            else if (radiostate == 0)
                            {
                                if (this.m_From != null)
                                    this.m_From.SendMessage(GetText(this.m_From, 100258), this.m_Target.Name);  // "You decided against challenging {0}."
                            }
                            break;
                        }
                }
                    ...
                    default:
                        {
                            if (radiostate == 1)
                            {
                                ...
                                if (this.m_DuelLocation != null && !XmlPoints.DuelLocationAvailable(this.m_DuelLocation))
                                {
                                    SendText(this.m_Target, 200650, this.m_DuelLocation.Name); // "{0} is occupied."
                                    return;
                                }
                            
                                // make sure neither participant is in combat
                                if (CheckCombat(this.m_From))
                                {
                                    SendText(this.m_From, 100214, this.m_Target.Name); // "Challenge with {0} has been cancelled"
                                    SendText(this.m_From, 100670, this.m_From.Name);  // "{0} is in combat."
                                    SendText(this.m_Target, 100214, this.m_From.Name); // "Challenge with {0} has been cancelled"
                                    SendText(this.m_Target, 100670, this.m_From.Name);  // "{0} is in combat."
                                    return;
                                }
                            
                                // make sure neither participant is in combat
                                if (CheckCombat(this.m_Target))
                                {
                                    SendText(this.m_From, 100214, this.m_Target.Name); // "Challenge with {0} has been cancelled"
                                    SendText(this.m_From, 100670, this.m_Target.Name);  // "{0} is in combat."
                                    SendText(this.m_Target, 100214, this.m_From.Name); // "Challenge with {0} has been cancelled"
                                    SendText(this.m_Target, 100670, this.m_Target.Name);  // "{0} is in combat."
                                    return;
                                }

                                XmlPoints a = (XmlPoints)XmlAttach.FindAttachment(this.m_From, typeof(XmlPoints));

                                // first confirm that they dont already have a challenge going
                                if (a != null && !a.Deleted && (a.Challenger != null || a.ChallengeGame != null))
                                {
                                    SendText(this.m_Target, 100261, this.m_From.Name); // "{0} has already been challenged."
                                
                                    SendText(this.m_From, 100262);  // "You are already being challenged."
                                    return;
                                }

                                XmlPoints ta = (XmlPoints)XmlAttach.FindAttachment(this.m_Target, typeof(XmlPoints));

                                // first confirm that they dont already have a challenge going
                                if (ta != null && !ta.Deleted && (ta.Challenger != null || ta.ChallengeGame != null))
                                {
                                    SendText(this.m_Target, 100262);  // "You are already being challenged."

                                    SendText(this.m_From, 100261, this.m_Target.Name);  // "{0} has already been challenged."
                                    return;
                                }
                                ...
                                if (a != null && !a.Deleted)
                                {
                                    a.Challenger = this.m_Target;
                                }

                                // assign the challenger field on the target points attachment
                                if (ta != null && !ta.Deleted)
                                {
                                    ta.Challenger = this.m_From;
                                }

                                // notify the challenger and set up noto
                                SendText(this.m_From, 100263, this.m_Target.Name);   // "{0} accepted your challenge!"
                                this.m_From.Send(new MobileMoving(this.m_Target, Notoriety.Compute(this.m_From, this.m_Target)));

                                // update the points gump if it is open
                                if (this.m_From.HasGump(typeof(PointsGump)))
                                {
                                    // redisplay it with the new info
                                    if (a != null && !a.Deleted)
                                        a.OnIdentify(this.m_From);
                                }

                                // notify the challenged and set up noto
                                SendText(this.m_Target, 100264, this.m_From.Name);  // "You have accepted the challenge from {0}!"
                                this.m_Target.Send(new MobileMoving(this.m_From, Notoriety.Compute(this.m_Target, this.m_From)));
                            
                                // update the points gump if it is open
                                if (this.m_Target.HasGump(typeof(PointsGump)))
                                {
                                    // redisplay it with the new info
                                    if (ta != null && !ta.Deleted)
                                        ta.OnIdentify(this.m_Target);
                                }

                                // cancel any precast spells
                                this.m_Target.Spell = null;
                                this.m_Target.Target = null;
                                this.m_From.Spell = null;
                                this.m_From.Target = null;

                                // let the challenger pick the dueling site
                                if (TeleportOnDuel && this.m_DuelLocation != null)
                                {
                                    Point3D duelloc = this.m_DuelLocation.DuelLocation;
                                    Map duelmap = this.m_DuelLocation.DuelMap;
                                    a.m_StartingLoc = this.m_From.Location;
                                    a.m_StartingMap = this.m_From.Map;
                                    ta.m_StartingLoc = this.m_Target.Location;
                                    ta.m_StartingMap = this.m_Target.Map;
                                    this.m_Target.MoveToWorld(duelloc, duelmap);
								
                                    // move over by 1
                                    duelloc.X += 1;
                                    this.m_From.MoveToWorld(duelloc, duelmap);
                                }
                                else
                                {
                            ...
                            {
                                SendText(this.m_From, 100265, this.m_Target.Name); // "Your challenge to {0} was declined."
                                SendText(this.m_Target, 100266, this.m_From.Name);  // "You declined the challenge by {0}."
                            }
                            break;
                        }
                }
