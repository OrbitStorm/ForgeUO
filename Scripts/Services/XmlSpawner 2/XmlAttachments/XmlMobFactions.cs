...
namespace Server.Engines.XmlSpawner2
{
    public class XmlMobFactions : XmlAttachment
    {
        ...
        private static readonly double m_CreditScale = 0.001;
        // these are the Members that define group members
        private static readonly Type[] PlayerMembers = new Type[]
        {
            typeof(PlayerMobile)
        };
        private static readonly Type[] HumanoidMembers = new Type[]
        {
        ...
        private static readonly Type[] UndeadMembers = new Type[]
        {
        ...
        private static readonly Type[] ReptilianMembers = new Type[]
        {
        ...
        private static readonly Type[] ArachnidMembers = new Type[]
        {
        ...
        private static readonly Type[] ElementalMembers = new Type[]
        {
        ...
        private static readonly Type[] AbyssMembers = new Type[]
        {
        ...
        static readonly int baseValue = 180000;// increasing this increases the faction effect on delayed acquisition. Higher=longer average delay
        static readonly int maxFaction = 25000;// this sets the faction level above which mobs will no longer acquire the target
        private static readonly Hashtable GroupTypeHash = new Hashtable();
        private static readonly Hashtable GroupHash = new Hashtable();
        ...
        public XmlMobFactions(ASerial serial)
            : base(serial)
        {
        }
        ...
        public XmlMobFactions(string name, double refractory)
        {
            ...
            this.FactionList = new GroupStatus[(int)GroupTypes.End_Unused];
            for (int i = 0; i < this.FactionList.Length; i++)
            {
                this.FactionList[i] = new GroupStatus();
                this.FactionList[i].GroupType = (GroupTypes)i;
            }
        }

        [Attachable]
        public XmlMobFactions()
            : this("Standard", 0)
        {
        }

        [Attachable]
        public XmlMobFactions(double refractory)
            : this("Standard", refractory)
        {
        }
        ...
        public static Group[] KillGroups
        {
            get
            {
                return m_KillGroups;
        ...
        public int Player
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Player);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Player, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Humanoid
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Humanoid);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Humanoid, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Undead
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Undead);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Undead, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Reptilian
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Reptilian);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Reptilian, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Arachnid
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Arachnid);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Arachnid, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Elemental
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Elemental);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Elemental, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Abyss
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Abyss);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Abyss, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int DragonLords
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.DragonLords);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.DragonLords, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int NecroMasters
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.NecroMasters);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.NecroMasters, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Fairie
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Fairie);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Fairie, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Plant
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Plant);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Plant, value);
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        public int Underworld
        {
            get
            {
                return this.GetFactionLevel(GroupTypes.Underworld);
            }
            set
            {
                this.SetFactionLevel(GroupTypes.Underworld, value);
            }
        }
        // ------------------------------------------------------------------------------
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // version 0
            writer.Write(this.verboseMobFactions);
            if (this.FactionList != null)
            {
                writer.Write(this.FactionList.Length);
                for (int i = 0; i < this.FactionList.Length; i++)
                {
                    GroupStatus g = this.FactionList[i];
            ...
            writer.Write(this.m_EndTime - DateTime.Now);
        }
        ...
        private static bool DynamicMatchType(ArrayList array, Mobile m)
        {
            ...
            foreach (object o in array)
            {
                if (o is XmlDynamicFaction && XmlDynamicFaction.MatchFaction(m, ((XmlDynamicFaction)o).Name))
                    return true;
            }
            return false;
        }
        ...
        {
            private readonly XmlMobFactions m_attachment;
            private readonly string m_text;
            public MobFactionGump(Mobile from, XmlMobFactions a, string text)
                : base(0,0)
            {
                if (a == null)
                    return;
                ...
                this.AddBackground(0, 0, 400, 330, 5054);
                this.AddAlphaRegion(0, 0, 400, 330);
                this.AddLabel(20, 2, 55, "Mob Faction Standings");

                if (a != null)
                {
                    this.AddLabel(200, 2, 30, String.Format("Available Credits: {0}", a.Credits));
                }

                this.AddHtml(20, 20, 360, 260, text, true, true);
                
                // add the verbose factions checkbox
                this.AddLabel(50, 290, 55, "Verbose Factions");
                this.AddButton(20, 290, (a.verboseMobFactions ? 0xD3 : 0xD2), (a.verboseMobFactions ? 0xD2 : 0xD3), 100, GumpButtonType.Reply, 0);
            }
            ...
            public override void OnResponse(NetState state, RelayInfo info)
            {
                ...
                {
                    case 100:
                        this.m_attachment.verboseMobFactions = !this.m_attachment.verboseMobFactions;
                    
                        state.Mobile.SendGump(new MobFactionGump(state.Mobile, this.m_attachment, this.m_text));
                        break;
                }
            }
        }
