    ...
    public class XmlAttach
    {
        ...
        public class AddAttCommand : BaseCommand
        {
            public AddAttCommand()
            {
                ...
                this.Commands = new string[] { "AddAtt" };
                this.ObjectTypes = ObjectTypes.Both;
                this.Usage = "AddAtt type [args]";
                this.Description = "Adds an attachment to the targeted object.";
                ...
                if (e != null && list != null && e.Length >= 1)
                {
                    ...
                    if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachment)))
                    {
                        ...
                        for (int i = 0; i < list.Count; ++i)
                        {
                            ...
                            if (o == null)
                            {
                                this.AddResponse(String.Format("Unable to construct {0} with specified args", attachtype.Name));
                                break;
                            }
                            ...
                            if (XmlAttach.AttachTo(null, list[i], o, true))
                            {
                                if (list.Count < 10)
                                {
                                    this.AddResponse(String.Format("Added {0} to {1}", attachtype.Name, list[i]));
                                }
                                count++;
                            }
                            else
                                this.LogFailure(String.Format("Attachment {0} not added to {1}", attachtype.Name, list[i]));
                        }
                        if (count > 0)
                        {
                            this.AddResponse(String.Format("Attachment {0} has been added [{1}]", attachtype.Name, count));
                        }
                        else
                        {
                            this.AddResponse(String.Format("Attachment {0} not added", attachtype.Name));
                        }
                    }
                    else
                    {
                        this.AddResponse(String.Format("Invalid attachment type {0}", e.Arguments[0]));
                    }
                }
            }
        }
        ...
        public class DelAttCommand : BaseCommand
        {
            public DelAttCommand()
            {
                this.AccessLevel = AccessLevel.GameMaster;
                this.Supports = CommandSupport.All;
                this.Commands = new string[] { "DelAtt" };
                this.ObjectTypes = ObjectTypes.Both;
                this.Usage = "DelAtt type";
                this.Description = "Deletes an attachment on the targeted object.";
                this.ListOptimized = true;
            }
            ...
            public override void ExecuteList(CommandEventArgs e, ArrayList list)
            {
                if (e != null && list != null && e.Length >= 1)
                {
                    ...
                    if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachment)))
                    {
                        ...
                        for (int i = 0; i < list.Count; ++i)
                        {
                            ...
                            if (alist != null)
                            {
                                ...
                                foreach (XmlAttachment a in alist)
                                {
                                    ...
                                    if (list.Count < 10)
                                    {
                                        this.AddResponse(String.Format("Deleted {0} from {1}", attachtype.Name, list[i]));
                                    }
                                    count++;
                                }
                            }
                        ...
                        if (count > 0)
                        {
                            this.AddResponse(String.Format("Attachment {0} has been deleted [{1}]", attachtype.Name, count));
                        }
                        else
                        {
                            this.AddResponse(String.Format("Attachment {0} not deleted", attachtype.Name));
                        }
                    }
                    else
                    {
        ...
        private class AttachTarget : Target
        {
            ...
            private readonly string m_set = null;
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (this.m_e.Arguments.Length > 0)
                {
                    type = SpawnerType.GetType(this.m_e.Arguments[0]);
                }
                if (this.m_e.Arguments.Length > 1)
                {
                    name = this.m_e.Arguments[1];
                }
                ...
                switch (this.m_set)
                {
                    case "add":
                        ...
                        // create a new attachment and add it to the item
                        int nargs = this.m_e.Arguments.Length - 1;

                        string[] args = new string[nargs];

                        for (int j = 0; j < nargs; j++)
                        {
                            args[j] = (string)this.m_e.Arguments[j + 1];
                        }

                        XmlAttachment o = null;

                        Type attachtype = SpawnerType.GetType(this.m_e.Arguments[0]);
                        ...
                        if (o != null)
                        {
                            ...
                            if (XmlAttach.AttachTo(from, targeted, o, true))
                                from.SendMessage("Added attachment {2} : {0} to {1}", this.m_e.Arguments[0], targeted, o.Serial.Value);
                            else
                                from.SendMessage("Attachment not added: {0}", this.m_e.Arguments[0]);
                        }
                        else
                        {
                            from.SendMessage("Unable to construct attachment {0}", this.m_e.Arguments[0]);
                        }
        ...
        private class ListAttachmentsGump : Gump
        {
                ...
                : base(X, Y)
            {
                ...
                this.AddBackground(20, 0, 330, 480, 5054);

                this.AddPage(1);

                this.AddImageTiled(20, 0, 330, 480, 0x52);

                this.AddLabel(27, 2, 0x384, "Available Attachments");
                this.AddHtml(25, 22, 320, 458, attachmentlist, false, true);
            }
        }
        ...
        private class DisplayAttachmentGump : Gump
        {
                ...
                : base(X, Y)
            {
                ...
                this.AddBackground(0, 0, 400, 150, 5054);
                this.AddAlphaRegion(0, 0, 400, 150);
                this.AddLabel(20, 2, 55, "Attachment Description(s)");

                this.AddHtml(20, 20, 360, 110, text, true, true);
            }
        }
    }
}