...
namespace Server.Engines.XmlSpawner2
{
    public class HandSiegeAttack : XmlAttachment
    {
        ...
        private readonly int m_currentdirection;
        ...
        public Point3D TargetLoc
        {
            get
            {
                return this.m_targetloc;
            }
            set
            {
                this.m_targetloc = value;
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        ...
        public void BeginAttackTarget(Mobile from, Item target, Point3D targetloc)
        {
            ...
            {
                this.CurrentLoc = from.Location;
                this.CurrentMap = from.Map;
                this.TargetLoc = target.Location;
        ...
        public void DoTimer(TimeSpan delay, bool wait)
        {
            ...
            if (this.m_Timer != null && this.m_Timer.Running && wait)
                return;
        ...
        {
            private readonly Item m_weapon;
            private readonly HandSiegeAttack m_attachment;
            ...
            {
                if (from == null || this.m_weapon == null || from.Map == null || this.m_attachment == null)
                    return;
                ...
                if (targeted is StaticTarget)
                {
                    ...
                    if (multiitem != null)
                    {
                        ...
                        this.m_attachment.BeginAttackTarget(from, multiitem, multiitem.Map.GetPoint(targeted, true));
                    }
                }
                else if (targeted is AddonComponent)
                {
                    ...
                    if (a == null || a.Deleted)
                    {
                        this.m_attachment.BeginAttackTarget(from, ((AddonComponent)targeted).Addon, ((Item)targeted).Location);
                    }
                    else
                    {
                        this.m_attachment.BeginAttackTarget(from, (Item)targeted, ((Item)targeted).Location);
                    }
                }
        ...
        private class InternalTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                Item weapon = this.m_attachment.AttachedTo as Item;
                Item target = this.m_attachment.AttackTarget;
                ...
                if (attackerloc != this.m_attachment.CurrentLoc || attackermap != this.m_attachment.CurrentMap)
                {
                    this.Stop();
                    return;
                }
                ...
                this.m_attachment.DoTimer(TimeSpan.FromSeconds(basedelay), false);
            }
        }
    }
}