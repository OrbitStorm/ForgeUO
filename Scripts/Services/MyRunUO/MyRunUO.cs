...
namespace Server.Engines.MyRunUO
{
    public class MyRunUO : Timer
    {
        ...
        protected override void OnTick()
        {
            ...
            try
            {
                shouldExit = this.Process(DateTime.Now + TimeSpan.FromSeconds(CpuInterval * CpuPercent));

                if (shouldExit)
                    Console.WriteLine("MyRunUO: Database statements compiled in {0:F2} seconds", (DateTime.Now - this.m_StartTime).TotalSeconds);
            }
            catch (Exception e)
            {
