	...
	public class ToolbarEdit : Gump
	{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 9:		// Default
					{
						List<string> toolbarinfo = ToolbarInfo.DefaultEntries(m.AccessLevel);
		...
		public void AddEntries()
		{
			...
			for (int i = 1; i <= 9; i++)
			{
				...
				********/
				...
				for (int l = 0; l < 15; l++)
				{
					AddTextEntry(38 + buffer + ((l % 3) * ((buffer * 2) + entryX)), 157 + ((int)Math.Floor((double)l / 3) * (entryY + 2)), entryX - 1, entryY, 2101, temp + 13, _Info.Entries[temp] /*,int size*/);
