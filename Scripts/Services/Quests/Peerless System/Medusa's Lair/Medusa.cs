...
namespace Server.Mobiles
{
    ...
    public class Medusa : BaseSABosses, ICarvable
    {
        ...
        public override void OnThink()
        {
            ...
            if (DateTime.Now > this.m_DelayOne)
            {
                Medusa.AffectedMobiles.Clear();

                this.m_DelayOne = DateTime.Now + TimeSpan.FromSeconds(Utility.RandomMinMax(30, 180));
            }

            if (DateTime.Now > this.m_GazeDelay)
            {
                ...
                if ((target != null) && !(target is MedusaClone) && !(target is StoneMonster))
                {
                    ...
                    this.m_GazeDelay = DateTime.Now + TimeSpan.FromSeconds(Utility.RandomMinMax(30, 60));
        ...
        public void SpawnStone(Mobile target)
        {
            ...
            if (DateTime.Now > this.m_DelayTwo)
            {
                int stones = 0;
                ...
                this.m_DelayTwo = DateTime.Now + TimeSpan.FromSeconds(Utility.RandomMinMax(30, 150));
            }
        }
