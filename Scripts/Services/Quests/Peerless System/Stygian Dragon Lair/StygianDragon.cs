...
namespace Server.Mobiles
{
    ...
    public class StygianDragon : BaseSABosses
    {
        ...
        {
            if (DateTime.Now > this.m_Delay)
            {
                switch (Utility.Random(2))
                {
                    case 0:
                        Ability.FlameCross(this);
                        this.m_Delay = DateTime.Now + TimeSpan.FromSeconds(Utility.RandomMinMax(25, 35));
                        break;
                    case 1:
                        Ability.CrimsonMeteor(this, 35);
                        this.m_Delay = DateTime.Now + TimeSpan.FromSeconds(Utility.RandomMinMax(20, 45));
                        break;
                }
            }
