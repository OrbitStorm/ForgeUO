...
namespace Server.Accounting
{
	public class Account : IAccount, IComparable, IComparable<Account>
	{
		...
		public Account(string username, string password)
		{
			...
			this.m_Created = this.m_LastLogin = DateTime.Now;
		...
		public bool Inactive
		{
			get 
			{
				...
				TimeSpan inactiveLength = DateTime.Now - this.m_LastLogin;
			...
			get
			{
				for (int i = 0; i < this.m_Mobiles.Length; i++)
				{
					...
					if (m != null && m.NetState != null)
						return this.m_TotalGameTime + (DateTime.Now - m.SessionStart);
				}
