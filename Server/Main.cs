...
namespace Server
{
	...
	public static class Core
	{
		...
		public static bool T2A
		{
			get
			{
				return m_Expansion >= Expansion.T2A;
			}
		}
		...
		public static bool UOR
		{
			get
			{
				return m_Expansion >= Expansion.UOR;
			}
		}
		...
		public static bool UOTD
		{
			get
			{
				return m_Expansion >= Expansion.UOTD;
			}
		}
		...
		public static bool LBR
		{
			get
			{
				return m_Expansion >= Expansion.LBR;
			}
		}
		...
		public static bool SA
		{
			get
			{
				return m_Expansion >= Expansion.SA;
			}
		}
		...
		public static float CyclesPerSecond
		{
			get
			{
				return m_CyclesPerSecond[(m_CycleIndex - 1) % m_CyclesPerSecond.Length];
			}
		}
		...
		public static void Main(string[] args)
		{
			...
			Console.WriteLine("AdaptUO - [http://www.adaptuo.com] Version {0}.{1}, Build {2}.{3}", ver.Major, ver.Minor, ver.Build, ver.Revision);
		...
		private static int m_ItemCount, m_MobileCount, _CustomsCount;
		...
		private static readonly Type[] _CustomsSerialTypeArray = new Type[1] { typeof(CustomSerial) };
		...
		private static void VerifyType(Type t)
		{
			...
			{
				Interlocked.Increment(ref _CustomsCount);
	...
	public class MultiTextWriter : TextWriter
	{
		...
		{
			this.m_Streams = new List<TextWriter>(streams);
		...
		public override void Write(char ch)
		{
			for (int i = 0; i < this.m_Streams.Count; i++)
				this.m_Streams[i].Write(ch);
		}

		public override void WriteLine(string line)
		{
			for (int i = 0; i < this.m_Streams.Count; i++)
				this.m_Streams[i].WriteLine(line);
		}
