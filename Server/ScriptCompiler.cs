...
namespace Server
{
    public static class ScriptCompiler
    {
        ...
        public static bool Compile(bool debug, bool cache)
        {
            ...
            Utility.PushColor(ConsoleColor.Green);
            Console.WriteLine("Finished ({0} items, {1} mobiles, {3} customs) ({2:F2} seconds)", Core.ScriptItems, Core.ScriptMobiles, watch.Elapsed.TotalSeconds, Core.ScriptCustoms);
            Utility.PopColor();
                    ...
                    if (attrs != null && attrs.Length > 0)
                    {
                        ...
                        if (attr != null)
                        {
                            for (int j = 0; j < attr.Aliases.Length; ++j)
                                this.m_FullNames.Add(attr.Aliases[j], type);
                        }
                    }
                }
            }
    ...
    public class TypeTable
    {
        ...
        public Type Get(string key, bool ignoreCase)
        {
            ...
            if (ignoreCase)
                this.m_Insensitive.TryGetValue(key, out t);
            else
                this.m_Sensitive.TryGetValue(key, out t);
        ...
        {
            this.m_Sensitive = new Dictionary<string, Type>(capacity);
            this.m_Insensitive = new Dictionary<string, Type>(capacity, StringComparer.OrdinalIgnoreCase);
        }
    }
}