...
namespace Server
{
    public static class World
    {
        ...
        public readonly static string DataIndexPath = Path.Combine("Saves/Customs/", "SaveData.idx");
        public readonly static string DataTypesPath = Path.Combine("Saves/Customs/", "SaveData.tdb");
        public readonly static string DataBinaryPath = Path.Combine("Saves/Customs/", "SaveData.bin");
            ...
            if (failedItems || failedMobiles || failedGuilds || failedData)
            {
                ...
                if (!Core.Service)
                {
                    ...
                    if (Console.ReadKey(true).Key == ConsoleKey.Y)
                    {
                        ...
                        SaveIndex<DataEntry>(DataIndexPath, data);
                    }
        ...
        {
            List<SaveData> results = new List<SaveData>();
        ...
        public sealed class DataEntry : ICustomsEntry
        {
            ...
            public CustomSerial Serial
            {
                get
                {
                    return this._Data == null ? CustomSerial.MinusOne : this._Data.Serial;
                }
            }
            public int TypeID
            {
